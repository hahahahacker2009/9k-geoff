 ...
 * and the Relist itself (but not the array of Relist pointers it's in).
 */
static void
_regfreelist(Relist *p, int elems)
{
	int i;
...
void
_regfreelists(Reljunk *j)
{
	if (j == nil)
		return;
	_regfreelist(j->relist[0], j->elems);
	_regfreelist(j->relist[1], j->elems);
	j->relist[0] = j->relist[1] = nil;
	j->reliste[0] = j->reliste[1] = nil;
	j->elems = 0;
}
