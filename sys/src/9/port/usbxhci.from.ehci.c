 ...
 * If the xhci is 64-bit capable it assumes we are using those
...
/*
 * Software. Xhci descriptors provided by pool.
...
Ecapio* xhcidebugcapio;
int xhcidebugport;

void
xhcirun(Ctlr *ctlr, int on)
{
	int i;
	Eopio *opio;
	...
	for(i = 0; i < 100; i++)
		if(on == 0 && (opio->sts & Shalted) != 0)
			...
			delay(1);
	if(i == 100)
		print("xhci %#p %s cmd timed out\n",
			ctlr->physio, on ? "run" : "halt");
	ddprint("xhci %#p cmd %#lux sts %#lux\n",
...
schedq(Ctlr *ctlr, Qh *qh, int pollival)
{
	...
	ddqprint("xhci: sched %#p q %d, ival %d, bw %uld\n",
		qh->io, q, pollival, bw);
	if(q < 0){
		print("xhci: no room for ed\n");
		return -1;
	}
	ctlr->tree->bw[q] += bw;
...
unschedq(Ctlr *ctlr, Qh *qh)
{
	...
	if(this == nil)
		print("xhci: unschedq %d: not found\n", q);
	else{
		next = this->next;
		qhlinkqh(prev, next);
	}
	...
	print("xhci: unschedq: qh %#p not found\n", qh);
}
...
qhalloc(Ctlr *ctlr, Ep *ep, Qio *io, char* tag)
{
	...
	case Tintr:
		...
		break;
	default:
		print("xhci: qhalloc called for ttype != ctl/bulk\n");
	}
	iunlock(ctlr);
	return qh;
}
...
qhcoherency(Ctlr *ctlr)
{
	...
	dprint("xhci: qhcoherency: doorbell %d\n", qhadvanced(ctlr));
	if(i == 3)
		print("xhci: async advance doorbell did not ring\n");
...
dump(Hci *hp)
{
	...
	opio = ctlr->opio;
	ilock(ctlr);
	print("xhci port %#p frames %#p (%d fr.) nintr %d ntdintr %d",
...
xhciintr(Hci *hp)
{
	...
	ctlr->nintr++;
	if((sts & Sherr) != 0)
		iprint("xhci: port %#p fatal host system error\n", ctlr->physio);
	if((sts & Shalted) != 0)
		iprint("xhci: port %#p: halted\n", ctlr->physio);
	if((sts & Sasync) != 0){
		dprint("xhci: doorbell\n");
	...
	if((sts & (Serrintr|Sintr)) != 0){
		ctlr->ntdintr++;
		if(xhcidebug > 1){
			iprint("xhci port %#p frames %#p nintr %d ntdintr %d",
		...
		do{
			if (qh == nil)
				panic("xhciintr: nil qh");
			if(qh->state == Qrun)
				some += qhinterrupt(ctlr, qh);
			qh = qh->next;
		}while(qh != ctlr->qhs && ++i < 100);
		if(i >= 100)
			iprint("xhci: interrupt: qh loop?\n");
		if (i > 0)
			intrtypes.qh++;
	}
	if (some == 0)
		intrtypes.no_work++;
//		panic("xhciintr: no work");
	iunlock(ctlr);
	return some;
}

/*
 * on the think server 140, we get millions of interrupts for
 * some other device on the same irq (usually the xhci controller;
 * disabling all the USB 3 ports in the bios should stop that).
 * dismiss those as quickly as possible.  i don't think we need to lock here.
 */
static Intrsvcret
...
portenable(Hci *hp, int port, int on)
{
	...
	dprint("xhci %#p port %d enable=%d; sts %#x\n",
	...
	dprint("xhci %#p port %d enable=%d: sts %#lux\n",
...
portlend(Ctlr *ctlr, int port, char *ss)
{
	...
	dprint("xhci %#p port %d: %s speed device: no longer owned\n",
		ctlr->physio, port, ss);
...
portreset(Hci *hp, int port, int on)
{
	...
	dprint("xhci %#p port %d reset; sts %#lux\n", ctlr->physio, port, *portscp);
	ilock(ctlr);
	/* Shalted must be zero, else Psreset will stay set */
	if (opio->sts & Shalted)
		iprint("xhci %#p: halted yet trying to reset port\n",
			ctlr->physio);
	...
	*portscp &= ~Psreset;		/* terminate reset */
	...
	for(i = 0; *portscp & Psreset && i < 10; i++)
		delay(10);
	if (*portscp & Psreset)
		iprint("xhci %#p: port %d didn't reset within %d ms; sts %#lux\n",
			ctlr->physio, port, i * 10, *portscp);
	delay(5);			/* xhci spec: enable on within 2 ms. */

	if((*portscp & Psenable) == 0)
		portlend(ctlr, port, "full");

	iunlock(ctlr);
	dprint("xhci %#p after port %d reset; sts %#lux\n",
		ctlr->physio, port, *portscp);
	qunlock(&ctlr->portlck);
	poperror();
	return 0;
...
portstatus(Hci *hp, int port)
{
	...
	if(s & (Psstatuschg | Pschange)){
		opio->portsc[port-1] = s;
		coherence();
		ddprint("xhci %#p port %d status %#x\n", ctlr->physio, port, s);
	}
	/*
...
episohscpy(Ctlr *ctlr, Ep *ep, Isoio* iso, uchar *b, long count)
{
	...
	for(tot = 0; iso->tdi != iso->tdu && tot < count; tot += nr){
		...
		if(tot + nr > count)
			nr = count - tot;
		if(nr == 0)
			print("xhci: ep%d.%d: too many polls\n",
				ep->dev->nb, ep->nb);
		else{
...
episofscpy(Ctlr *ctlr, Ep *ep, Isoio* iso, uchar *b, long count)
{
	...
	for(tot = 0; iso->stdi != iso->stdu && tot < count; tot += nr){
		...
		if(stdu->csw & Stdactive){
			diprint("xhci: episoread: %#p tdu active\n", iso);
			break;
		}
		nr = stdu->ndata;
		if(tot + nr > count)
...
episoread(Ep *ep, Isoio *iso, void *a, long count)
{
	...
	iso->debug = ep->debug;
	diprint("xhci: episoread: %#p ep%d.%d\n", iso, ep->dev->nb, ep->nb);
	...
	while(isocanread(iso) == 0){
		iunlock(ctlr);
		diprint("xhci: episoread: %#p sleep\n", iso);
		if(waserror()){
			if(iso->err == nil)
...
episowrite(Ep *ep, Isoio *iso, void *a, long count)
{
	...
	iso->debug = ep->debug;
	diprint("xhci: episowrite: %#p ep%d.%d\n", iso, ep->dev->nb, ep->nb);
	...
	for(tot = 0; tot < count; tot += nw){
		while(isocanwrite(iso) == 0){
			iunlock(ctlr);
			diprint("xhci: episowrite: %#p sleep\n", iso);
			if(waserror()){
				if(iso->err == nil)
					iso->err = "I/O timed out";
				ilock(ctlr);
	...
	if(err != nil)
		error(err);
	diprint("xhci: episowrite: %#p %d bytes\n", iso, tot);
	return tot;
}
...
epgettd(Qio *io, int flags, void *a, int count, int maxpkt)
{
	...
	if(count > Tdmaxpkt)
		panic("xhci: epgettd: too many bytes");
	td = tdalloc();
...
xhcipoll(void* a)
{
	...
	for(;;){
		if(ctlr->nreqs == 0){
			if(0)ddprint("xhcipoll %#p sleep\n", ctlr->physio);
			sleep(poll, workpending, ctlr);
			if(0)ddprint("xhcipoll %#p awaken\n", ctlr->physio);
		}
...
pollcheck(Hci *hp)
{
	...
	if(poll->must != 0 && poll->does == 0){
		...
		if(poll->must != 0 && poll->does == 0){
			poll->does++;
			print("xhci %#p: polling\n", ctlr->physio);
			kproc("xhcipoll", xhcipoll, hp);
		}
		unlock(poll);
	}
}
...
epiowait(Hci *hp, Qio *io, int tmout, ulong load)
{
	...
	ctlr = hp->aux;
	qh = io->qh;
	ddqprint("xhci %#p: io %#p sleep on qh %#p state %s\n",
		ctlr->physio, io, qh, qhsname[qh->state]);
	timedout = 0;
	if(waserror()){
		dqprint("xhci %#p: io %#p qh %#p timed out\n",
			ctlr->physio, io, qh);
		timedout++;
	}else{
		if(tmout == 0)
	...
	if(qh->state == Qrun){
		...
		ilock(ctlr);
		if(qh->state == Qdone){
			dqprint("xhci %#p: polling required\n", ctlr->physio);
			ctlr->poll.must = 1;
			pollcheck(hp);
		}
	}
	...
	if(qh->state == Qrun){
//		dqprint("xhci %#p: io %#p qh %#p timed out (no intr?)\n",
		iprint("xhci %#p: io %#p qh %#p timed out (no intr?)\n",
			ctlr->physio, io, qh);
		timedout = 1;
	}else if(qh->state != Qdone && qh->state != Qclose)
		panic("xhci: epio: queue state %d", qh->state);
	if(timedout){
		aborttds(io->qh);
...
epio(Ep *ep, Qio *io, void *a, long count, int mustlock)
{
	...
	if((xhcidebug > 1 || ep->debug > 1) && io->tok != Tdtokin){
		seprintdata(buf, buf+sizeof(buf), a, count);
		print("xhci epio: user data: %s\n", buf);
	}
	if(mustlock){
		qlock(io);
		if(waserror()){
	...
	ddeprint("xhci: load %uld ctlr load %uld\n", load, ctlr->load);
	if(xhcidebug > 1 || ep->debug > 1)
		dumptd(td0, "epio: put: ");
...
epctlio(Ep *ep, Ctlio *cio, void *a, long count)
{
	...
	if(c[Rtype] & Rd2h){
		...
		if(len > Maxctllen)
			error("d2h data too large to fit in xhci");
...
epwrite(Ep *ep, void *a, long count)
{
	...
	ddeprint("xhci: epwrite ep%d.%d\n", ep->dev->nb, ep->nb);
	if(ep->aux == nil)
		panic("xhci: epwrite: not open");
	switch(ep->ttype){
	case Tctl:
		cio = ep->aux;
...
isoopen(Ctlr *ctlr, Ep *ep)
{
	...
	if(ctlr->load + ep->load > 800)
		print("usb: xhci: bandwidth may be exceeded\n");
	ilock(ctlr);
	ctlr->load += ep->load;
	ctlr->isoload += ep->load;
	ctlr->nreqs++;
	dprint("xhci: load %uld isoload %uld\n", ctlr->load, ctlr->isoload);
...
epopen(Ep *ep)
{
	...
	ctlr = ep->hp->aux;
	deprint("xhci: epopen ep%d.%d\n", ep->dev->nb, ep->nb);
	if(ep->aux != nil)
		panic("xhci: epopen called with open ep");
	if(waserror()){
		free(ep->aux);
		ep->aux = nil;
		nexterror();
...
cancelio(Ctlr *ctlr, Qio *io)
{
	...
	qh = io->qh;
	dqprint("xhci: cancelio for qh %#p state %s\n",
		qh, qhsname[qh->state]);
	aborttds(qh);
	qh->state = Qclose;
	iunlock(ctlr);
...
epclose(Ep *ep)
{
	...
	ctlr = ep->hp->aux;
	deprint("xhci: epclose ep%d.%d\n", ep->dev->nb, ep->nb);

	if(ep->aux == nil)
		panic("xhci: epclose called with closed ep");
	switch(ep->ttype){
	case Tctl:
		cio = ep->aux;
		cancelio(ctlr, cio);
...
mkqhtree(Ctlr *ctlr)
{
	...
	ctlr->ntree = i;
	dprint("xhci: tree: %d endpoints allocated\n", i);
...
#ifdef TODO_
	dprint("xhci %#p flb %#lux frno %#lux\n",
...
init(Hci *hp)
{
	...
	xhcirun(ctlr, 1);
	/*
	 * it's not obvious how multiple xhcis can work, and on some machines,
	 * setting Callmine on all xhcis makes the machine seize up.
	 */
	if(getconf("*firstxhci") != nil)
		/*
		 * this is a broken machine, so route all ports by default
		 * to only one xhci (the first).
		 */
