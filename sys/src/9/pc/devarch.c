...
getcpuids(void)
{
	...
	if (!(m->cpuiddx & Cpuapic))
		panic("cpu0: no lapic; should retire this pre-1994 antique");
	if (!(m->cpuiddx & Cpumsr))
		panic("cpu0: no msrs; should retire this pre-1993 antique");
	if (!(m->cpuiddx & Tsc))
		panic("cpu0: no tsc; should retire this pre-1993 antique");
	...
	if (cpuidexthigh >= Extmiscfeat) {
		memset(regs, 0, sizeof regs);
		cpuid(Extmiscfeat, regs);
		invartsc = regs[3] & Invartsc;	/* dx */
	}
	/* pcs have had invariant tscs since 2008 */
	if (!invartsc)
		print("time-stamp counter NOT invariant; "
			"be sure Turbo Boost is disabled in BIOS\n");
}
