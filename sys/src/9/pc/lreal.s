 ...
 *	- we're called from C, so general regs used have been saved
 */
...
TEXT now16real(SB), $0
	/* load the registers from Ureg at RMUADDR for the bios call */
	CLR(rAX)
	MTSR(rAX, rSS)
...
TEXT realmodeintrinst(SB), $0
	...
	/* store the registers into Ureg at RMUADDR from the bios call */
