...
addbus(int type, int bno)
{
	...
	dprint("addbus: new bus %d type %d\n", bno, type);
...
checkmtrr(void)
{
	...
	/*
	 * These print on the apu2, but just reflect re-ordered mtrrs,
	 * so don't bother and hope for the best.
	 */
if(0)	for(i = 0; i < vcnt; i++)
...
mpstartap(Apic* apic)
{
	...
	*p = i>>8;
	...
	/* shutdown code: warm reset upon init ipi */
