...
enum {
	 ...
	 * so allow for that.
	 */
	Iowaitms    = 5*60*1000, /* generous; should be done in under 1 s. */
	Iopktwaitms = Iowaitms,	/* 6 s. wasn't enough */
...
westerndigitalhung(Drive *drive)
{
	if((drive->feat&Datapi) == 0 && drive->active && drive->intick &&
	    TK2MS(sys->ticks - drive->intick) > Iowaitms/2){
...
awaitspinup(Drive *drive)
{
	...
	print("awaitspinup: %s didn't spin up after 60 seconds\n", name);
	iunlock(drive);
}
	...
	uvlong lba, int *tryp)
{
	...
	if (task & (Efatal<<8) ||
		...
		iprint("%s: unhappy drive; recovering\n", drive->unit->name);
		drive->port->ci = 0;
		ahcirecover(drive);
