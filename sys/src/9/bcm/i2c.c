...
enum {
	...
	I2cRead	= 1<<0,		/* read transfer */
	I2cWrite	= 0<<0,		/* write transfer */
...
i2cio(int rw, int tenbit, uint addr, void *buf, int len, int salen, uint subaddr)
{
	...
	p = buf;
	st = rw == I2cRead? Rxd : Txd;
	while(len > 0){
		while((r->stat & (st|Done)) == 0){
			r->ctrl |= rw == I2cRead? Intr : Intt;
...
{
	i2cio(I2cWrite, d->tenbit, d->addr, buf, len, d->salen, offset);
	return len;
}
...
{
	i2cio(I2cRead, d->tenbit, d->addr, buf, len, d->salen, offset);
	return len;
}
