...
{
	ulong entry, text, data, bss, magic, all, pentry, off;
	uchar *sdata;
	Exechdr *hdr;
	 ...
	 * on 386, entry (e.g., 0xe0100020) is at offset 0x20 in file.
	 * on amd64, entry (e.g., 0xffffffff80110000) is at offset 0x28 in file.
	 */
	off = sizeof(Exec);
	if (magic & HDR_MAGIC)
		off += sizeof hdr->uvl;
	all = off + text + data;
	if(unzip((uchar *)KADDR(PADDR(entry)) - off, all,
...
readlzip(Boot *b)
{
	/* the whole lzipped kernel is now at b->bp */
