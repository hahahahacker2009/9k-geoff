/*
 * Synopsys DWC sdio host controller
 */
...
#define unitregs(subno)	(void *)(soc.sdmmc + PGSZ*(subno))

/*
 * on visionfive 2, freebsd uses transfer rates of 100 MHz for mmc
 * (hs200 timing), 50 MHz for sd.
 */
enum {
	Maxunit		= 1,		/* number of slots */
	Maxdev		= 2,		/* number of devices */
	/* linux says min freq is 100000 and max is 200000000 */
...
dmaalloc(void *addr, int len)
{
	...
	desclen = (n * sizeof(Adma) + CACHELINESZ) & ~(uintptr)(CACHELINESZ-1);
	if(desclen <= sdhost.admalen)
		adma = sdhost.adma;
	else{
		free(sdhost.adma);
		adma = mallocalign(desclen, CACHELINESZ, 0, 0);
		sdhost.adma = adma;
	...
	cachedwbinvse(adma, (char*)p - (char*)adma);
	return adma;
}
...
static int
sdhostinquiry(int subno, char *inquiry, int inqlen)
{
	USED(subno);
...
static int
sdhostcmd(int subno, u32int cmd, u32int arg, u32int *resp)
{
	u32int *r;
	u32int c;
	int i;
	...
	/* why are we seeing Ctimeout on visionfive 2? */
...
void
sdhostiosetup(int subno, int write, void *buf, int bsize, int bcount)
{
	u32int *r;
	Adma *adma;
	int len;
...
static void
sdhostio(int subno, int write, uchar *buf, int len)
{
	u32int *r;
	int mask;
...
static void
sdhostinterrupt(Ureg*, void *sdhostp)
{	
	u32int *r;
	int i;
