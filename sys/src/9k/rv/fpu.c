...
fpupostnote(void)
{
	...
	snprint(n, sizeof(n), "sys: fp: %s Exception fsw=%#ux", msg, fsw);
...
fptrap(Ureg* ureg, void*)
{
	...
	if(up == nil)
		panic("fptrap: fpu in kernel: pc %#p", ureg->pc);
	...
	if(ureg->pc & KZERO) {
		...
		iprint("fptrap: kernel pc %#p proc %d %s up->fpustate %d\n",
	 ...
	 */
	fpison = (ureg->status & Fsst) != (Off<<Fsshft);
	if (fpison)
		up->fcsr = getfcsr();
	...
	if (fpison) {
		iprint("trap: ill fp inst %#lux with enabled fp status %lld\n",
			(ulong)ureg->tval, ureg->status & Fsst);
