...
enum Privmodes {		/* plic context offsets for priv modes */
...
typedef int (*Traphandler)(Cause *);
typedef void (*Exchandler)(Cause *);
...
static void debugbpt(Cause*);
static void dumpgpr(Ureg* ureg);
static void faultriscv64(Cause *);
static void trapsyscall(Cause *);
...
plicinit(void)
{
	...
	if (maxpri == 0) {
		plicoff();
		iprint("plic: %#p: max priority 0, ignoring\n", plic);
		soc.plic = 0;
		return;
	}
	sys->maxplicpri = maxpri;
	if (soc.newmach)
		iprint("plic: %#p: max prio %#ux\n", plic, maxpri);
 ...
 */
static int
traplocalintr(Cause *cp)
{
	vlong excgrp, vno, cause;

	m->intr++;			/* okay here; only tmr and sw intrs */
	m->perf.intrts = perfticks();
	cause = cp->cause;
	if (cause == 0)			/* probably NMI */
		trapnmi(cp);
	excgrp = cause >> Excgrpshft;
	if (excgrp == (Suptmrintr >> Excgrpshft))
		clocksvc(cp->ureg);
	else if (excgrp == (Supswintr >> Excgrpshft)) {
		/*
...
//		procipimsgs();
	...
	intrtime(m, vno);
	if (!soc.plic)
		poll(cp);		/* frequent polling */
	return excgrp;			/* groups are ipi then timer */
}

static void
trapmisaligned(Cause *cp)
{
	if (cp->user)
		posttrapnote(cp->ureg, cp->cause, "misaligned access");
	else
		panic("misaligned access to %#p at %#p",
			cp->ureg->tval, cp->ureg->pc);
}
...
{
	if (cp->user)
		posttrapnote(cp->ureg, cp->cause, "illegal instruction");
	else
		panic("illegal instruction at %#p: %#p",
			cp->ureg->pc, cp->ureg->tval);
}
...
trapillinst(Cause *cp)
{
	...
	/* the remaining instructions that we recognise are not compressed. */
...
{
	if (cp->user)
		posttrapnote(cp->ureg, cp->cause, "illegal access");
	else
		panic("trap: cpu%d: illegal access to %#p at %#p; epc %#p",
			m->machno, cp->ureg->tval, cp->ureg->pc, csrrd(SEPC));
}
...
trapriscv64(Cause *cp)
{
	...
	if (handler == nil)
		panic("unknown exception, cause %d from_user %d",
			cause, cp->user);
	(*handler)(cp);
	if (Trapdebug)
		faultstuck(ureg);
...
void
poll(Cause *cp)		/* desperate last resort */
{
	int vno;
	Vctl *v;
	...
	for (v = pollvecs; v != nil; v = v->pollnxt)
		if (v->f) {
			...
			(*v->f)(cp->ureg, v->a);	/* fake an interrupt */
			intrtime(m, vno);
...
#define TRAPDBG(cp, c) if (!Trapdebug) {} else trapdbg(cp, c)
...
#ifdef MEASURE_FAULTS
typedef struct Faultcnts Faultcnts;
struct Faultcnts {		/* page fault counts by addresses and PCs */
	uvlong	faults;
	uintptr	vaddr;
	uintptr	pc;
	/* could add cpu and pid */
};

Faultcnts faultcnts[1024];
Lock cntlock;

static void
countfaults(uintptr addr, uintptr pc)
{
	Faultcnts *fc, *efc;

	if ((pc & KZERO) == 0)	/* fault from user pc?  ignore it. */
		return;

	/* find a matching entry, or an empty one, if there's room */
	efc = faultcnts + nelem(faultcnts);
	ilock(&cntlock);
	for (fc = faultcnts; fc < efc; fc++) {
		if (fc->vaddr == addr && fc->pc == pc)
			break;
		if (fc->vaddr == 0 && fc->pc == 0) {
			fc->vaddr = addr;
			fc->pc = pc;
			break;
		}
...
static int
fltcmp(void *a, void *b)
{
	return ((Faultcnts *)b)->faults - ((Faultcnts *)a)->faults;
}
...
prcntfaults(void)
{
	...
	ilock(&cntlock);
	qsort(faultcnts, nelem(faultcnts), sizeof *faultcnts, fltcmp);
	iunlock(&cntlock);
	efc = faultcnts + nelem(faultcnts);
	for (fc = faultcnts, i = 0; fc < efc && i < 20; fc++, i++) {
		if (fc->faults < 10 || fc->vaddr == 0 && fc->pc == 0)
			 break;
		iprint("%lld\t%#p\tpc %#p\n", fc->faults, fc->vaddr, fc->pc);
	}
}

void	(*prfaults)(void) = prcntfaults;

#else

#define countfaults(addr, pc)
#endif

/*
 *  find out fault address and type of access.
 *  Call common page fault handler.
 */
static void
faultriscv64(Cause *cp)
{
	uintptr addr;
	int read, insyscall;
	Ureg *ureg;
	...
	if (Trapdebug && addr == 0 && up->nlocks)	/* debugging for gs */
		panic("fault %#llux pc %#p addr %#p nlocks %d",
			ureg->cause, ureg->pc, addr, up->nlocks);

	insyscall = up->insyscall;
	up->insyscall = 1;
	read = ureg->cause != Storepage;  /* exception, so Rv64intr is 0 */
	/* page fault on a kernel address is never okay. */
	if((intptr)addr < 0 || fault(addr, read) < 0)
		badpagefault(ureg, addr, read, insyscall);
	up->insyscall = insyscall;

	countfaults(addr, ureg->pc);
	vecacct(vctl[cp->vno]);
}
