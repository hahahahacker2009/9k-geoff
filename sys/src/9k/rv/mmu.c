...
{
	if (*pte)
		panic("mappage: pte for va %#p in use: %#llux\n", va, *pte);
	if (va < KZERO)
		ptroot->daddr++;
	/* super page leaf */
	*pte = PTEPPN(pa) | PteRWX | Pteleafvalid | va2gbit(va);
}
...
mmulowidmap(void)
{
	...
	page = m->ptroot;
	/* zero user mapping */
	zero((void *)page->va, sizeof(PTE) * page->daddr);
	page->daddr = 0;

	pgsz = PGLSZ(Toplvl);	/* on Sv39, 1GB; on Sv48, 512GB */
...
/*
 * Allocate and minimally populate new page tables & Mach for machno.
 *
...
cpusalloc(int ncpus)
{
	...
	for(machno = 1; machno < ncpus && machno < MACHMAX; machno++)
		newcpupages(machno);
}
...
golow(void)
{
	...
	if (sys->nonline <= 1)
		sys = ensurelow(sys);
	m = ensurelow(m);
	m->ptroot->va = (uintptr)ensurelow(m->ptroot->va);
...
mmuflushtlb(Page *)
{
	...
	ptroot = m->ptroot;
	if (ptroot == nil) {
		iprint("mmuflushtlb: cpu%d: m->ptroot not yet set\n", m->machno);
		splx(s);
	}
	if(ptroot->daddr){
		/* zero the top-level user-space PTEs */
		clrreserv();
		zero((char *)ptroot->va, ptroot->daddr*sizeof(PTE));
		ptroot->daddr = 0;
	}
	wbinvd();
	splx(s);
...
mmuptpalloc(void)
{
	...
	if (((uintptr)va & KZERO) == 0)
		panic("mmuptpalloc: va %#p is physical", va);
	page->va = PTR2UINT(va);
	page->pa = PADDR(va);
	page->ref = 1;
	return page;
...
mmuswitch(Proc* proc)
{
	...
	ptroot = m->ptroot;
	pte = (PTE *)ptroot->va;
	if (((uintptr)pte & KZERO) == 0)
		panic("mmuswitch: ptroot->va %#p is physical", pte);
...
/* allocate page table for level l w/ index x, insert it into pte. */
static Page *
 ...
 * may allocate page table pages with allocpt().
 * called from mmuput and mmuwalk, so beware recursion filling in tables.
 * mapped sizes are, from level 0: 4k, 2m, 1g, 512g, 128t.
 *
 ...
 * each PTE identifies itself as leaf or interior node, and mapped size is
 * implled by level, so the trick doesn't seem to be feasible.
 */
static PTE*
mmuptpget(uintptr va, int level, int *lvlgotp)
{
	...
	if (m == nil) {
		iprint("mmuptpget: nil m\n");
		return nil;
	}
	if (m == nil || m->ptroot == nil) {
		iprint("mmuptpget: cpu%d: nil m->ptroot\n", m->machno);
		return nil;
	}
	prev = m->ptroot;
	ptp = (PTE *)prev->pa;			/* Toplvl */
	for (l = Toplvl; (lvlgot = l) > 0; ) {	/* level 0 has to be a leaf */
		if (ptp == nil)
		...
		if (Debugptpget)
			iprint("mmuptpget: va %#p lvl %d pt is va %#p: %#p\n",
				va, l, ptp, pte);
		/*
		 * walk toward a leaf PTE from the level-after-root until
		 * `level', return the containing page.
		 */
		if (--l < level)
			break;
		if (!PAGEPRESENT(pte)) {	/* parent hole? */
			 ...
			 */
			prev = allocpt(l, idx, &kptp[idx], prev);
			pte = kptp[idx];
			ptnxt = nextpte(pte);	/* physical */
		}
 ...
 * may use allocate page tables with allocpt() or mmuptpget().
 * only called from fault.c.  requires a user context (up != nil).
 */
void
mmuput(uintptr va, uintmem pa, Page *page)
{
	Mpl pl;
	int l, lvlgot, x;
	uintptr ova, opa;
...
mmudumplvl(uintptr ptroot, uintptr base, int lvl)
{
	...
	pte = (PTE *)ptroot;
	indent(Toplvl - lvl);
	iprint("page table level %d at %#p:", lvl, pte);
	lastpte = -1;
	lastcont = 0;
	for (i = 0; i < PGSZ/sizeof(PTE); i++, lastpte = ent) {
		ent = pte[i];
		ptepa = PADDRINPTE(ent);
		if (PTEATTRBITS(ent) == PTEATTRBITS(lastpte) &&
		    ptepa > PADDRINPTE(lastpte)) {
		...
		indent(Toplvl - lvl);
		/* root page table is split between low and high addresses */
		if (lvl == Toplvl && i >= Ptpgptes/2)
			va = base + (i-Ptpgptes/2)*PGLSZ(lvl) + KZERO;
		else
			va = base + i*PGLSZ(lvl);
		iprint("pte @ va %#10p idx %3d for va %#10p -> pa %#10p ",
			&pte[i], (int)i, va, ptepa);
		for (bp = bitnames; bp < bitnames + nelem(bitnames); bp++)
			iprint("%c", ent & bp->bit? bp->name: '-');
		iprint(" ");
		if (ent & PteLeaf)
			iprint("%spage leaf", lvl > 0? "super-": "");
		else {
			iprint("next level ptr:\n");
			mmudumplvl((uintptr)KADDR(ptepa), va, lvl-1);

			indent(Toplvl - lvl);
			iprint("----");
			lastcont = 0;
		}
		if (ptepa & m->pgszmask[lvl] != 0)
			iprint(", not aligned on (super) page boundary");
	}
	iprint("\n");
}
...
void
mmudump(uintptr ptroot, int toplvl)
{
...
earlypagealloc(void)
{
	uintptr pa;
	Sys *mysys;
	extern Sys *lowsys;		/* from low.c */

	mysys = lowsys;
	if (mysys->freepage == nil)
		mysys->freepage = mysys->pts;
	if (mysys->freepage >= (Ptepage *)mysys->pts[nelem(mysys->pts)]) {
		pa = asmwalkalloc(PGSZ);
		return KADDR(pa);
	}
	return (PTE *)mysys->freepage++;
}
 ...
 *
 * we use mallocalign() or earlypagealloc() as appropriate.
 */
PTE *
mmunewpage(PTE *pde)
{
	...
	palloc = alloc;
	if (iskern(alloc))
		palloc = (void *)PADDR(alloc);
	mmuwrpte(pde, PTEPPN(palloc) | va2gbit((uintptr)alloc) | PteP);
	return alloc;
}

/*
 * only called from pdmap and its subordinates, so it's okay to call
 * mmunewpage() to allocate page table pages.
 */
static PTE *
 ...
 * mmuptpget() or mmunewpage() may allocate new page table pages.
 */
static PTE *
...
/*
 * only called from pdmap, so it's okay to call mmunewpage to allocate page
 * table pages.  call with *pde == 0 .
 */
static void
...
/* only called from pdmap.  may allocate page table pages with mmuptpget(). */
static void
 ...
 * mmuwalk() or mmuptpget() may allocate page table pages.
...
vmap(uintmem pa, uintptr size)
{
	...
	paend = pa + sz - 1;
	if (iskern(pa) && iskern(paend)) /* already in kernel space? */
...
/*
 * walk page tables for va from ptroot, return corresponding pa.
 */
uintptr
mmutrans(PTE *ptroot, uintptr va)
{
	int idx, l;
	uintptr pa;
	PTE pte;
	...
	for(l = Toplvl; l >= 0; l--){
		...
		pte = kptp[idx];			/* root page pte */
		if(!PAGEPRESENT(pte)) {
			iprint("no pte for %#p at level %d\n", va, l);
			break;
		}
		pa = PADDRINPTE(pte);
		if (l > 0 && pte & PteLeaf)		/* super page leaf? */
			pa += va & m->pgszmask[l];	/* low bits */
//		iprint("pte %#p: va %#p level %d %#p[%d] -> pa %#p\n",
//			pte, va, l, kptp, idx, pa);
		if (l > 0 && pte & PteLeaf)		/* super page leaf? */
			break;
	}
	return pa;
...
/*
 * walk page tables for va from root to level.  returns actual level or -1.
 * return final PTE* via ret.
 * allocate new page table pages as needed, via mmuptpget() or (*alloc)().
 */
int
...
/**/	putsatp(pagingmode | (ptroot->pa / PGSZ));
	...
	DBG("mach cpu%d: m %#p; pt root va %#p -> pa %#p\n", m->machno, m,
		ptroot->va, ptroot->pa);
	/* jumphigh already called by low() */
