...
enum {
	 ...
	 * were usually ample; however systems can need more receive buffers
...
rballoc(void)
{
	...
	if((bp = rbpool) != nil){
		...
		adec(&nrbavail);
		if (nrbavail < 0)
			print("ethergem rballoc: nrbavail negative\n");
	}
	iunlock(&rblock);
	if (bp)
...
transmit(Ether *edev)
{
	...
	if(nqd) {
		ctlr->tdt = tdt;
		ctlr->regs[Txsts] |= Txdone|Txusedbitread;	/* reset bits */
		coherence();
		ctlr->txtick = sys->ticks;
	}
	ctlr->regs[Ctl] |= Txstart;		/* kick transmitter */
	ienablelcked(ctlr, Txdoneint);
	iunlock(&ctlr->reglock);
	qunlock(&ctlr->tlock);
}
...
tim(void *vc)
{
	...
	return ctlr->tdba[ctlr->tdt].size_flags & Sent;
}
...
rxinit(Ctlr *ctlr)
{
	...
	regs[Pcsctl] |= Pcsenan;
	/* could do a software reset here */
	cfg = regs[Cfg];
	...
	/* set up the rcv ring */
	setmacs(ctlr);			/* in case sw reset cleared it */
...
rim(void *vc)
{
	...
	return ctlr->rdba[ctlr->rdh].addr & Full;
}
...
qinpkt(Ctlr *ctlr)
{
	...
	if ((ctlr->regs[Rxsts] & Rxdonebits) == Rxrcvd){
		/* Rd says no overrun and buffer available */
		if (bp == nil) {
...
#define GETMACREGS(regs) \
	(((uvlong)(regs)[Specaddr1top] & MASK(16))<<32 | (regs)[Specaddr1bot])
...
setmacregs(Ctlr *ctlr)
{
	...
	addr = ctlr->macregs;
	regs = ctlr->regs;
	macaddr = GETMACREGS(regs);
	if (etherismacset(addr) && !etherismacset(macaddr)) {
		regs[Specaddr1bot] = addr;
		regs[Specaddr1top] = addr>>32;
	}
}
...
reset(Ctlr *ctlr)
{
	...
	if (soc.newmach)
		iprint("resetting ethergem, we hope...");
	if(detach(ctlr)){
...
discover(void)
{
	...
	*(ulong *)SOFT_RESET &= ~(Ether0); /* ether1 out of reset */
	...
	for (i = 0; i < nelem(soc.ether); i++)
		newctlr(soc.ether[i]);		/* ether0 is actually mac 1 */
}
