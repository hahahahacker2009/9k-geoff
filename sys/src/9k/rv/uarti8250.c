...
enum {					/* Ier */
	...
	Exsu		= 1<<6,		/* xscale unit enable, mandatory */
};
...
i8250kick(Uart* uart)
{
	...
	if(i < 128 && (csr8r(ctlr, Lsr) & Temt))
		...
		ctlr->sticky[Ier] |= Ethre;
	csr8w(ctlr, Ier, soc.xscaleuart? Exsu: 0);
}
...
i8250disable(Uart* uart)
{
	...
	ctlr = uart->regs;
	ctlr->sticky[Ier] = 0;
	if (soc.xscaleuart)		/* e.g., milk-v jupiter */
		ctlr->sticky[Ier] |= Exsu; /* xscale unit enable, mandatory */
	csr8w(ctlr, Ier, ctlr->sticky[Ier]);
...
i8250console(char* cfg)
{
	...
	if (!uart->enabled)
		(*uart->phys->enable)(uart, 0);	/* 2nd arg is int enable */
	if (Change_speed)
