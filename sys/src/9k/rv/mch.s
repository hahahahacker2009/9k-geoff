 ...
 * bypasses the uncompressed instruction and sets m->probebad.
 */
...
//	MOV	normalsatp(SB), R9		/* in case we botched satp */
//	MOV	R9, CSR(SATP)
//	SFENCE_VMA(0, 0)
...
/*
 * jump to upper address range and adjust addresses in registers (m, sp, sb).
 ...
 */
/*
 * approximate intel's wbinvd instruction on systems with coherent dma: i & d
 * fences, flush mmu state.  doesn't flush caches.
 */
TEXT wbinvd(SB), 1, $-4
...
TEXT invlpg(SB), 1, $-4
	...
	/* sifive u74 erratum cip-1200: can't do a non-global SFENCE. u54 too? */
	BARR_SFENCE_VMA(0, 0)
#else
...
#define CBOINVAL 0		/* Zicbom instructions */
#define CBOCLEAN 1		/* write back iff dirty */
#define CBOFLUSH 2		/* clean then invalidate */

#define CBOZERO 4		/* Zicboz */

/* Zicbom extension; takes virtual addresses and operates on all caches */

TEXT cboclean(SB), 1, $-4			/* void cboclean(uintptr) */
	WORD $(0xf | 2<<12 | ARG<<15 | CBOCLEAN<<20)
	RET
...
/* Zicboz extension; takes virtual addresses */

TEXT cbozero(SB), 1, $-4			/* void cbozero(uintptr) */
	WORD $(0xf | 2<<12 | ARG<<15 | CBOZERO<<20)
	RET
