...
#define BROKEN		0	/* for "if (BROKEN)" */
#define WTF		0	/* for "if (WTF)" */
...
#ifndef BLOCKALIGN		/* must be CACHELINESZ on incoherent systems */
#define BLOCKALIGN	8
#endif
...
/*
 * Greatest Mach stack use observed is 2504 bytes.
...
/*
 * PHYSMEM is physical address of start of RAM that may be occupied by the
 * kernel; membanks[0].size is the usable memory directly above that.  The
 * memory at PHYSMEM must be readable and writable by the kernel, so must skip
 * any unmapped or protected firmware.  This default base address (0x80000000)
 * is likely when a risc-v machine has M & S modes (due to Unix conventions).
 * PHYSMEM must not be zero, which is the nil pointer, in practice if not in
 * K&R1.
 *
 * We usually choose BANK0SIZE so that it ends at the first GB above PHYSMEM,
 * thus putting Sys 1MB below, and the reboot trampoline code at the default
 * RBKTZERO, 0xbff05000.  The overrides go in <machine-type>/defs.
 */
#ifndef PHYSMEM
#define PHYSMEM		0x80000000ull		/* cached dram */
#endif
 ...
 * Should be in port.  But see riscv64.h Pte*.
 */
#define PTEVALID	(1<<0)
#define PTERONLY	(1<<1)
#define PTEWRITE	(1<<2)
...
/*
 * this was intended for pages of alternating colors, not numa clusters.
 * use m->color? Page->color?
 * mips used (((uintptr)(va) >> PIDXSHFT) % NCOLOR)
 */
