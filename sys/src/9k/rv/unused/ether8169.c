...
#define csr8r(regs, r)	(coherence(), *((uchar *) (regs)+(r)))
#define csr16r(regs, r)	(coherence(), *((ushort *)(regs)+((r)/2)))
#define csr32p(regs, r)	((ulong *)   (regs)+((r)/4))
#define csr32r(regs, r)	(coherence(), *csr32p(regs, r))

#define csr8w(regs, r, b)  (*((uchar *) (regs)+(r)) = (b), coherence())
#define csr16w(regs, r, w) (*((ushort *)(regs)+((r)/2)) = (w), coherence())
#define csr32w(regs, r, v) (*csr32p(regs, r) = (v), coherence())
...
rtl8169mii(Ctlr* ctlr)
{
	...
	if(mii(ctlr->mii, (1<<1)) == 0 || (phy = ctlr->mii->curphy) == nil){
		...
		free(ctlr->mii);
		ctlr->mii = nil;
print("rtl8169: no mii.  macv %#ux phyv %#ux\n", ctlr->macv, ctlr->phyv); // TODO
		return -1;
	}
...
rtl8169reset(Ctlr* ctlr)
{
	...
	/* turn on goddamned emac0 clocks on jupiter */
	ulong *gdclks = (ulong *)(0xd4282800 + 0x3e4);
	*gdclks = ~0;
	coherence();
	delay(10);

	int reg;
	for (reg = 0; reg < 0x1000; reg += sizeof(ulong)) {
		r = csr32r(regs, reg);
		if ((r & 0xffffff) == 0xfefefe)
			iprint("%#ux: %#lux\n", reg, r);
	}

	iprint("initial Cr %#ux\n", csr8r(regs, Cr));
	r = csr32r(ctlr->nic, Idr0);
	iprint("initial Idr0 %#lux\n", r);
	if ((r & 0xffffff) != 0xfefefe) {
		iprint("bad mac addr; not a real 8169\n");
		return -1;
	}
	iprint("initial Idr4 %#lux\n", csr32r(ctlr->nic, Idr0+4));
	...
	if(r & Rst) {
		iprint("8169: ctlr didn't reset, Rst still set in Cr %#lux\n",
			r);
		return -1;
	}
...
qpkt(Ether *edev, int rdh, ulong control)
{
	...
	/*
	 * invalidate caches for buffer after dma filled it, in case the
...
//				break;
			...
			if((control & (Fs|Ls|Res)) == (Fs|Ls)) {
				...
				passed++;
			} else if (badpkt(edev, rdh, control) == Ctlrreset)
...
newctlr(void *regs)
{
	...
	mem = vmap(io, PGSZ);
	if(mem == nil){
		print("#l%d: ether8169: can't map regs %#p\n", nctlr, regs);
		return nil;
	}
	...
	ctlr->pciv = 0;
	ctlr->pcie = 1;
print("8169: phys %#p virt %#p\n", io, mem);	// TODO
	vetmacv(ctlr, (uint *)&ctlr->macv);
	if(rtl8169reset(ctlr)){
		print("%Æ: can't reset\n", ctlr);
		free(ctlr);
		vunmap(mem, PGSZ);
