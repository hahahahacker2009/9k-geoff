...
etheriq(Ether* ether, Block* bp, int fromwire)
{
	...
	if (ether == 0)
		panic("etheriq: nil ether %#p", ether);
	ether->inpackets++;

	if (bp == 0)
		panic("etheriq: nil bp %#p", bp);
	pkt = (Etherpkt*)bp->rp;
	if (pkt == 0)
		panic("etheriq: nil bp->rp %#p", pkt);
	if (bp->wp == 0)
		panic("etheriq: nil bp->wp %#p", bp->wp);
	len = BLEN(bp);
	...
	for(fp = ether->f; fp < ep; fp++)
		if((f = *fp) != nil && (f->type == type || f->type < 0) &&
			...
			if(f->bridge && !fromwire && !fromme)
				...
				xbp->wp += len;
				if(qpass(f->iq, xbp) < 0)
					/* this is bad but don't log it */
					// softovflow(ether, "f->iq copy");
					ether->soverflows++;
			}

	if(fx){
		if(qpass(fx->iq, bp) < 0)
			softovflow(ether, "fx->iq normal");
		return nil;
	}
	if(fromwire){
		freeb(bp);
...
addethercard(char* t, int (*r)(Ether*))
{
	...
	if(ncard >= MaxEther)
		panic("addethercard: too many ether card types (%d)", ncard);
	cards[ncard].type = t;
	cards[ncard].reset = r;
	ncard++;
}
...
etherconffmt(char *p, char *e, Ether *ether, int cardno)
{
	...
	p = seprint(p, e, " regs %#p irq %d", ether->port,
		(ushort)(ether->irq & ~Attachenable));
	if(ether->mem)
...
etherprobe(int cardno, int ctlrno)
{
	...
	/*
	 * If ether->irq is zero, it indicates no interrupt used by
	 * ethersink.  Or perhaps the driver has some other way to configure
	 * interrupts for itself (e.g., MSI).
	 */
	if(ether->irq > 0 && !(ether->irq & Attachenable))
		enableintr(ether, ether->interrupt, ether, name);
	/* else enable in attach instead to delay interrupts for dwmac */
...
etherfmt(Fmt* fmt)
{
	...
	switch(fmt->r){
	case L'Æ':
	case L'æ':				/* compatibility with old */
		...
		/* 0 may be wrong, but we don't know better */
		ctlrno = ctlrcomm->edev == nil? 0: ctlrcomm->edev->ctlrno;
...
void
etherenableirqs(Ether *edev)		/* enable all irqs known to ioconf */
{
	int realirq, irq, nirqs;
	char name[16];
	Ioconf *ioc;

	if(edev->irq == 0 || !(edev->irq & Attachenable))
		return;			/* already done by reset */
	snprint(name, sizeof(name), "ether%d", edev->ctlrno);
	realirq = edev->irq & ~Attachenable;
	ioc = ioconf("ether", edev->ctlrno);
	nirqs = ioc? ioc->xirqs: 0;
	for (irq = realirq + 1; nirqs > 0; nirqs--, irq++) {
		edev->irq = irq;
		enableintr(edev, edev->interrupt, edev, name);
	}
	edev->irq = realirq;
	enableintr(edev, edev->interrupt, edev, name);	/* main irq */
}

/* is low 48 bits of addr non-zero and not all ones? */
int
etherismacset(uvlong addr)
{
	return addr == 0 || addr == VMASK(8*Eaddrlen)? 0: 1;
}

/*
 * returns true if we set the mac in ra to the default, from ether0mac.
 * regadd is the mac read from the device registers.
 */
int
ethersetmac(uchar *ra, int ctlrno, uvlong regadd)
{
	int i;
	...
	if (etherismacset(regadd)) {
		...
		return 0;
	} else {			/* regs cleared by reset or u-boot */
