/*
 * Synopsys Gb Ethernet Mac (GMAC) up to, but not including, v4.0.
 * v4 introduces incompatible changes.
 *
 * Only able to use the bottom 4GB.
 *
 * DMA is not cache coherent on Beagle V (StarFive JH7100); fixed in JH7110.
 *
 ...
 * VisionFive 2's is v5.20 (dwmac-5.20).
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
enum {				/* interesting dev registers, by ulong* index */
	...
	Addrhi0	= 0x40/4,
	Addrlo0	= 0x44/4,
	/* there are 15 more pairs of addresses, last 8 at 0x800 & 0x804 */
...
enum {				/* Rd->ctlcnts bits */
	...
	Altbuflenshft = 16,	/* for buffer 2 */
	Altbuflen = MASK(13)<<Altbuflenshft,
...
transmit(Ether *edev)
{
	...
	for(nqd = 0; NEXT(tdt, Ntd) != tdh; nqd++){	/* ring not full? */
		...
		cachedwbinvse(bp->rp, BLEN(bp));
		td = &ctlr->tdba[tdt];
		td->addr = PCIWADDR(bp->rp);
		if (Altdesc) {
			td->ctlcnts = BLEN(bp)<<Altbuflenshft;
			td->status = (td->status & Altteor) | Altintcompl |
				Altls | Altfs;
		} else
			td->ctlcnts = (td->ctlcnts & Eor) | Intcompl | Fseg |
				Ls | BLEN(bp)<<Buflenshft;
		cachedwbinvse(td, sizeof *td);
...
tim(void *vc)
{
	...
	if(qlen(ctlr->edev->oq) == 0)
		return 0;
	cachedwbinvse(&ctlr->tdba[ctlr->tdt], sizeof(Td));
...
initrxring(Ctlr *ctlr, Rd *rdbase, int ndescs)
{
	...
	for (i = 0; i < ndescs; i++) {
		...
		rd->ctlcnts = Rbsz << (Altdesc? Altbuflenshft: Buflenshft);
	}
	ctlr->rdba[ndescs - 1].ctlcnts |= (Altdesc? Altreor: Eor);
	coherence();
	cachedwbinvse(ctlr->rdba, ndescs * sizeof(Rd));
}
...
inittxring(Ctlr *ctlr, Td *tdbase, int ndescs)
{
	...
	coherence();
	cachedwbinvse(ctlr->tdba, ndescs * sizeof(Td));
}
...
replenish(Ctlr *ctlr)
{
	...
	for(rdt = ctlr->rdt; NEXT(rdt, Nrd) != rdh; rdt = NEXT(rdt, Nrd)){
		...
		*rb = bp = rballoc();
		...
		if (Altdesc)
			rd->ctlcnts = (rd->ctlcnts & Altreor) |
				Rbsz<<Altbuflenshft;
		else
			rd->ctlcnts = (rd->ctlcnts & Eor) | Intcompl |
				Rbsz<<Buflenshft;
		cachedwbinvse(rd, sizeof *rd);
...
rim(void *vc)
{
	...
	if(ctlr->rim != 0)
		return 1;
	cachedwbinvse(&ctlr->rdba[ctlr->rdh], sizeof(Rd));
...
setmacs(Ctlr *ctlr)
{
	...
	ra = ctlr->ra;
	ilock(&ctlr->reglock);
	regadd = (uvlong)regs[Addrhi0]<<32 | regs[Addrlo0];
	if (ethersetmac(ra, ctlrno, regadd)) {
		ctlrno++;
		iprint("%Æ: mac unset; setting to %E\n", ctlr, ra);
...
reset(Ctlr *ctlr)
{
	...
	if (soc.newmach)
		iprint("resetting ethersyngmac, we hope...");
	if(detach(ctlr)){
...
allocall(Ctlr *ctlr)
{
	...
	/* don't free resources; they may be reused */
