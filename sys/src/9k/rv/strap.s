...
TEXT strap(SB), 1, $-4
	...
	BEQ	R(MACH), trapaligned		/* should never happen */
	FENCE
...
fromuser:
	MOV	$recutrap(SB), R9
	MOV	R9, CSR(STVEC)		/* if we fault in strap, report it */
	...
	/* reserve stack space for pc, regs, csrs */
	ADD	$(KSTACK-UREGSIZE), R2	/* must be multiple of sizeof(vlong) */
	S(1)
	/*
	 * system calls (Envcalluser) don't need most registers saved
	 * because the C caller saved them; see below.
	 */
	S(ARG)				/* syscall number is in R(ARG) */

	MOV	CSR(SCAUSE), R9
	MOV	$Envcalluser, R4
	BEQ	R4, R9, syscallentry
	/* all faults from user mode other than syscalls */
	MOV	SAVESR9(R(MACH)), R9
	JAL	LINK, pushr2_31(SB)	/* save the lot */
syscallentry:
	/* patch up: copy old saved regs into Ureg on stack just loaded */
	MOV	SAVESR2(R(MACH)), R9;	MOV R9, (2*XLEN)(R2)
	MOV	SAVESR3(R(MACH)), R9;	MOV R9, (3*XLEN)(R2)
	MOV	SAVESR4(R(MACH)), R4;	MOV R4, (4*XLEN)(R2)
	MOV	SAVESR6(R(MACH)), R9; 	MOV R9, (6*XLEN)(R2)

	/* save CSRs, mostly for trap() */
	MOV	CSR(SEPC), R9;		MOV R9, 0(R2)	/* push saved pc */
	MOV	CSR(SSTATUS), R9;	MOV R9, (32*XLEN)(R2)
	MOV	CSR(SIE), R9;		MOV R9, (33*XLEN)(R2)
	MOV	CSR(SCAUSE), R9;	MOV R9, (34*XLEN)(R2)
	MOV	CSR(STVAL), R9;		MOV R9, (35*XLEN)(R2)
	MOV	$Mppsuper, R9;		MOV R9, (36*XLEN)(R2) /* curmode */
	MOV	CSR(SSCRATCH), R9;	MOV R9, (37*XLEN)(R2) /* old MACH */
...
/**/	CALLTRAP  /* C compiler is free to write regs but R2, SB, MACH, USER */
	/*
	 * USER might not be correct after resched, but we're returning to user,
	 * so it won't be used as is, but will be recomputed before next use.
	 */
...
syscallret:
	...
	P(ARG)				/* syscall return value from Ureg */

	MOV	(34*XLEN)(R2), R9	/* saved SCAUSE */
	MOV	$Envcalluser, R4
	BEQ	R4, R9, syscallexit
	JAL	LINK, popr8_31(SB)
syscallexit:
	MOV	(1*XLEN)(R2), R1	/* restore link reg */
	MOV	(4*XLEN)(R2), R4	/* restore saved j[acl] temporary */
	MOV	(5*XLEN)(R2), R5
	MOV	(6*XLEN)(R2), R6	/* restore user's R6 */
