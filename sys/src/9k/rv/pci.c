...
enum {
	...
	Port0		= 0,
	Port1		= 0x100000,	/* size of ecam space per bus */
...
static int pcimaxbno = 7;	/* can be up to 255 */
...
pcicfginit(void)
{
	...
	if (probeulong((ulong *)pci, Read) < 0) {
		iprint("pcicfginit: pci ctl va %#p faulted on read\n", pci);
		goto err;	/* stop trying; it's missing */
	}
	...
	pcicfgmode = 1;
	pcimaxdno = 7;		/* TODO: is 7 for p550, for speed */
	pcimaxbno = 3;		/* TODO: is 3 for p550, for speed */
 ...
 */
static void *
pcicfgaddr(int tbdf, int rno)
{
	uchar *addr;

	addr = soc.pci + Cfgspace + (BUSBDF(tbdf)<<4) + rno;
	if (addr >= soc.pcictl)
		panic("pcicfgaddr: addr out of range\n");
	return addr;
}
#endif
...
pcicfgrw16(int tbdf, int rno, int data, int read)
{
	...
	addr = pcicfgaddr(tbdf, rno);
//iprint("pcicfgrw16: addr %#p for %T %d\n", addr, tbdf, rno);
...
pcicfgrw32(int tbdf, int rno, int data, int read)
{
	...
	addr = pcicfgaddr(tbdf, rno);
//iprint("pcicfgrw32: addr %#p for %T %d\n", addr, tbdf, rno);
	lock(&pcicfglock);
	v = probeulong(addr, Read);
	if (v < 0)
		x = -1;
	else {
		v = *(ulong *)addr;
...
pcireset(void)
{
	...
	if (soc.pci == nil || soc.pcictl == nil) {
		print("pcireset: no pci addresses in soc\n");
		return;
	}
	if(pcicfgmode < 0)
		pcicfginit();
