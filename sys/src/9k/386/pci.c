...
pcibusmap(Pcidev *root, ulong *pmema, ulong *pioa, int wrreg)
{
	...
	table = smalloc(2*ntb*sizeof(Pcisiz));
	itb = table;
	mtb = table+ntb;
...
//				DBG("pci: ignored bridge %T\n", p->tbdf);
	...
	for(tptr = table; tptr < itb; tptr++) {
		...
		if(tptr->bar == -1)
			...
			p->pcr |= IOen;
			p->mem[tptr->bar].bar = ioa|Barioaddr;
			if(wrreg)
				pcicfgrw32(p->tbdf, PciBAR0+(tptr->bar*4),
					ioa|Barioaddr, 0);
		}
...
pcireservemem(void)
{
	 ...
	 * beware the type and size of 'Pcibar->bar'.
	 */
	for(p=pciroot; p; p=p->list)
		for(i=0; i<nelem(p->mem); i++) {
			bar = &p->mem[i];
			addr = bar->bar;
			if(addr && (addr & Barioaddr) == 0)
				asmmapinit(addr & Baraddrmask, bar->size, AsmDEV);
		}
}
