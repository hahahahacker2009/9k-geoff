...
/*
 * close the output half of a tcp connection, effectively signalling eof
 * while keeping the input half alive for responses.
 */
static char *
tcpxmitclose(Conv *c)
{
...
tcpiput(Proto *tcp, Ipifc*, Block *bp)
{
	...
	if((h4->vihl&0xF0)==IP_VER4) {
		...
		if(!(bp->flag & Btcpck) && (h4->tcpcksum[0] || h4->tcpcksum[1]) &&
		    (csum = ptclcsum(bp, TCP4_IPLEN, length-TCP4_IPLEN))) {
			tpriv->stats[CsumErrs]++;
			tpriv->stats[InErrs]++;
			netlog(f, Logtcp,
			    "bad tcpv4 proto cksum: from %I, got %#ux, computed %#ux\n",
				source, h4->tcpcksum[0]<<8 | h4->tcpcksum[1],
				csum);
			freeblist(bp);
			return;
		}
		...
		if((h6->tcpcksum[0] || h6->tcpcksum[1]) &&
			...
			tpriv->stats[InErrs]++;
			netlog(f, Logtcp,
			    "bad tcpv6 proto cksum: from %I, got %#ux, computed %#ux\n",
				source, h6->tcpcksum[0]<<8 | h6->tcpcksum[1],
				csum);
			freeblist(bp);
			return;
		}
...
tcpctl(Conv* c, char** f, int n)
{
	...
	if(n == 1 && strcmp(f[0], "close") == 0) {	/* needed by go */
		tcpclose(c);
		return nil;
	}
	...
	if(strcmp(f[0], "keepalive") == 0)
		return tcpstartka(c, f, n);
	if(strcmp(f[0], "checksum") == 0)
		return tcpsetchecksum(c, f, n);
	if(strcmp(f[0], "tcpporthogdefense") == 0)
