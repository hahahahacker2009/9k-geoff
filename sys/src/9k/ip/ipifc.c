...
enum {
	...
	/* flag: host 0 is broadcast. 4.2bsd did it, not wanted for decades */
	Netaddrisbcast	= 0,
	Classfulbcast	= 0,	/* flag: add old classful directed broadcasts */
};
...
ipifcadd(Ipifc *ifc, char **argv, int argc, int tentative, Iplifc *lifcp)
{
	...
	*l = lifc;
	...
	/* skip v6 loopback, it's a special address */
	if(ipcmp(ip, v6loopback) != 0 && ipcmp(mask, IPallbits) == 0)
		type |= Rptpt;
		...
		for(cp = f->ipifc->conv; cp < e; cp++){
			...
			for(lifc = nifc->lifc; lifc; lifc = lifc->next){
				...
				if(ipcmp(net, lifc->remote) == 0) {
					...
					arpenter(f, V6, ip, nifc->mac, Eaddrlen, 0);
