...
fpusave(PFPU *pfpu)
{
	if(pfpu->fpustate & Hold)
		panic("fpusave: Hold set in fpustate");
	return fpalign(pfpu->fxsave);
}
...
fpuidle(PFPU *p)
{
	_fxsave(fpusave(p));	/* relatively costly; dumps much state */
	m->fpsaves++;
	_stts();
	p->fpustate = Idle;
}
...
fpunm(Ureg* ureg, void*)
{
	...
	/*
	 * Someone tried to use the FPU in a note handler, which we now permit?
	 */
	pfpu = &up->PFPU;
	if(up->fpustate & Hold){
		pfpu = &up->notefpu;
	 ...
	 * state, not physical m->fcw.  - geoff
	 */
	if(ureg->ip & KZERO) {
	...
	case Idle:
		...
		save->fcw &= ~Sff;
		_clts();
		_fxrstor(save);  /* relatively costly; inhales much state */
		m->fprestores++;
		pfpu->fpustate = Busy;
		break;
	}
