 ...
 * adapting to i350:
 *	not known if this works on i350s on supermicro motherboards.
 *	earliest cards appear to be from 2006.
 * there is i219 support, but it's untested.
 *
 ...
 * be buggy anyway.  We only use one tx and one rx queue per interface,
 * which greatly simplifies the driver.
 *
 ...
 * VLANs are an issue between switches or routers; we needn't deal with them.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
enum {
	 ...
	 *
	 * systems can need more receive buffers due to bursts of (non-9P)
	 * traffic or slowness (e.g., apu2).
	 */
	Maxtd	= 56,		/* 57[12] erratum 70: at most 60 tds in q */
	Ntd	= 32,		/* too small just slows xmit */
	Nrd	= 64,		/* if too small, drops packets */
	/* 9boot's tftp/udp seems to need 1024 (was 384) on apu; ttftp doesn't */
	...
	Avoidreset = 0,		/* flag: leave pxe boot settings intact? */
	Touchqs	= 0,		/* flag: reset unused queues */
};
...
enum {
	...
	Eeer	= 0x0e30/4,	/* energy-efficient ethernet (eee) (i350) */
	...
	/*
	 * i350 extended interrupt registers for ivar queues.
	 * on the i350, Icr, etc. are aliased to 0x1500, etc.
	 *
	 * Intel smashed the 10GbE ivar queueing stuff into the i350,
	 * and the result is messy, with two sets of interrupt registers,
	 * normal (above) and extended (below).  extended is for ivar queues.
	 */
	Eics	= 0x1520/4,	/* extended interrupt Cause Set */
	Eims	= 0x1524/4,	/* " " Mask Set/Read: enabled causes */
	Eimc	= 0x1528/4,	/* " " mask Clear */
	Eiac	= 0x152c/4,	/* " " auto clear */
	Eiam	= 0x1530/4,	/* " " acknowledge Auto Mask */
	Eicr	= 0x1580/4,	/* " " Cause Read */
	Ivar	= 0x1700/4,	/* 4 intr vector allocation reg.s */
	...
	/*
	 * these are per queue, ctlr->nqueues at 0x100 spacing.  see Perq.
	 */
	...
	Vfre	= 0x0c8c/4,	/* VF receive enable */
	Srrctl	= 0xc00c/4,	/* split & replication receive control */
	...
	/* end of per-queue base regs */
	Tdwbal	= 0xe038/4,	/* Td write back base low */
	Tdwbah	= 0xe03c/4,	/* Td write back base high */
	...
	Vfte	= 0x0c90/4,	/* VF transmit enable */
...
/*
 * depending on ctlr, rx & tx regs may appear at 0x2800/0x3800 stride 0x100,
 * 0xc000/0xe000 stride 0x40.  queue 0 regs may appear at 0x110/0x420.
 * 0x2800/0x3800 seem to work everywhere for the first 4 queues;
 * the next 4 are at 0xc100/0xe180.  generality?  regularity?
 * intel's heard good things about them; maybe some day...
 */
struct Perq {			/* per-queue descriptor registers */
	ulong	dbal;		/* desc base addr low */
	ulong	dbah;		/* desc base addr high */
	ulong	dlen;		/* descs length (20 bits) */
	ulong	srrctl;		/* split & replic. rcv ctl */
	ulong	dh;		/* desc head; index of 1st valid */
	ulong	_2814;
	ulong	dt;		/* desc tail; index of 1st invalid */
	ulong	_2828[3];
	ulong	xdctl;		/* desc control */
};

enum {					/* Ctrl */
	Fd		= 1<<0,		/* full duplex */
	GIOmd		= 1<<2,	/* GIO (PCIe) master disable; self-clearing */
	Lrst		= 1<<3,		/* link reset */
	Slu		= 1<<6,		/* Set Link Up */
	...
	Swrst		= 1<<26,	/* sw reset; self-clearing */
...
enum {					/* Status */
	...
	GIOme		= 1<<19,	/* GIO (PCIe) Master Enable Status */
};

enum {					/* Eec */
	EEflsinuse	= 1<<6,	/* flash present & valid, needs EEautord */
	EEpres		= 1<<8,	/* flash or shadow ram preset, needs EEautord */
	EEautord	= 1<<9,	/* flash auto-read done */
...
enum {					/* interrupts: Icr, Ics, Ims, Imc */
	...
	/* means: got a packet, waited a bit, didn't get another */
	Rxt0		= 0x00000080,	/* Receiver Timer Interrupt */
	Rxdw		= Rxt0,		/* receiver desc write back (i350) */
	...
	Rxq0		= 1<<20,	/* Rx Queue 0 */
	Rxq1		= 1<<21,	/* Rx Queue 1 */
	Eccer		= 1<<22,	/* uncorrectable ECC error (i217) */
	Txq0		= 1<<22,	/* Tx Queue 0 */
	Txq1		= 1<<23,	/* Tx Queue 1 */
	Ths		= 1<<23,	/* thermal sensor event (i350) */
	Mddet		= 1<<28,	/* malicous driver detect (i350) */
	Intassert	= 1u<<31,	/* interrupt pending (i217) */
};

enum {
	/* extended interrupt causes (Eicr, etc.) for Ivar on i350 */
	Irx0bitno	= 0,
	Irx0		= 1<<Irx0bitno,	/* driver defined: rcv q knocking */
	Itx0bitno	= 1,
	Itx0		= 1<<Itx0bitno,	/* driver defined: tx q knocking */
	Otherint	= 1ul<<31,	/* see icr for non-q-related causes */
};
...
enum {					/* Rctl */
	...
	Rdtms½		= 0x00000000,	/* Threshold is 1/2 Rdlen */
	Rdtms¼		= 0x00000100,	/* Threshold is 1/4 Rdlen */
	Rdtms⅛		= 0x00000200,	/* Threshold is 1/8 Rdlen */
...
enum {					/* Tctl */
	...
	RrthreshSHIFT	= 29,
	RrthreshMASK	= 3<<RrthreshSHIFT,
};

enum {					/* Rfctl */
	Ipv6xsumdis	= 1<<11,	/* disable ipv6 checksum */
	Lef		= 1<<18,	/* accept packets w/ wrong length */
};
...
enum {					/* [RT]xdctl */
	...
	Gran		= 1<<24,	/* Granularity (descriptors, not cls) */

	/* Qenable, Swflush, Priority & Hwbthresh overlap LwthreshMASK */
	LwthreshSHIFT	= 25,
	LwthreshMASK	= MASK(7)<<LwthreshSHIFT, /* tx: descriptor low threshold (!575) */

	Qenable		= 1<<25,	/* Queue Enable (multiq ctlrs) */
	Swflush		= 1<<26,	/* (i210) */
	Priority	= 1<<27,	/* e.g., i350 */
	HwbthreshSHIFT	= 28,
	HwbthreshMASK	= MASK(4)<<HwbthreshSHIFT, /* tx: head writeback threshold (i350) */
};
...
enum {
	/* 16 (/2) and 32-bit (/4) flash registers for ich flash parts */
...
struct Rxtx {			/* experiment */
	uint	dt;		/* desc tail index (add descs here) */
	uint	dh;		/* " head index (hw filling/sending head) */
	Rd	*dba;		/* descs base address */
	Watermark wmd;
	Rendez	rendez;
	int	im;		/* interrupt cause bits from interrupt svc */
//	int	eim;		/* extended " (i350) */
	Block	**b;		/* pkt buffers */
	ulong	intr;
	ulong	sleeps;
};
...
struct Ctlr {
	Ethident;		/* first, per etherif.h */
	Pcidev	*pcidev;
	Ctlr	*next;		/* next in chain of discovered Ctlrs */
	uintptr	port;		/* used during discovery & printing */
	uchar	nqueues;	/* 1-8, so far */
	char	multiq;		/* flag: implements multiple queues */
	char	hasfutextnvm;
	...
	/* default: 0-x3f are for the sole port (lan0) */
	/* i210: 0-0x7f defined, but may be more */
	/* i350: 0-0x7f lan0, lan[1-3] 64-short blocks after */
	ushort	eeprom[0x40];
...
static Perq *
qrxregs(Ctlr *ctlr, uint n)
{
	if (n >= ctlr->nqueues)
		panic("qrxregs: queue %d out of range for %d queues",
			n, ctlr->nqueues);
	if (n < 4)			/* use compatible addresses */
		return (Perq *)((char *)ctlr->regs + 0x2800 + 0x100*n);
	else
		return (Perq *)((char *)ctlr->regs + 0xc000 + 0x40*n);
}

static Perq *
qtxregs(Ctlr *ctlr, uint n)
{
	if (n >= ctlr->nqueues)
		panic("qtxregs: queue %d out of range for %d queues",
			n, ctlr->nqueues);
	if (n < 4)			/* use compatible addresses */
		return (Perq *)((char *)ctlr->regs + 0x3800 + 0x100*n);
	else
		return (Perq *)((char *)ctlr->regs + 0xe000 + 0x40*n);
}
...
i82563ifstat(Ether* edev, void* a, long n, ulong offset)
{
	...
	spds = ctlr->speeds;
	p = seprint(p, e, "speeds: 10M:%ud 100M:%ud 1G:%ud ?:%ud\n",
...
#define PWTHRESH(n)	((n)<<WthreshSHIFT | (n)<<PthreshSHIFT)
#define PWHTHRESH(n)	((n)<<HthreshSHIFT | PWTHRESH(n))
...
settxdctl(Ctlr *ctlr)
{
	...
	regs[Txdctl] = r;
	if (ctlr->multiq)
		awaitbitpat(&regs[Txdctl], Qenable, Qenable);
	return r;
}
...
i82563txinit(Ctlr* ctlr)
{
	...
	case i217:
		/*
		 * osdev wiki says tctl must be exactly 0x3003f0fa:
		 * Ten | Psp | 15<<Ctshift | 63<<ColdSHIFT | Mulr |
		 *	1<<RrthreshSHIFT
		 * bit 28 (Mulr) is reserved, read-only and 1
		 */
		tctl |= 63<<ColdSHIFT | 1<<RrthreshSHIFT;
		break;
	default:
	case i218:
		tctl |= 66<<ColdSHIFT;
...
i82563transmit(Ether* edev)
{
	...
	/* are we interrupting yet? */
	if (ctlr->tintr == 0 && ctlr->rintr == 0 && ctlr->attticks &&
	    sys->ticks - ctlr->attticks > 60*HZ) {
		ctlr->attticks = 0;
		iprint("%Æ: no intrs in first minute since link up\n", ctlr);
	}
...
i82563replenish(Ctlr* ctlr)
{
	...
	for(rdt = ctlr->rdt; nxt = NEXT(rdt, Nrd), nxt != ctlr->rdh; rdt = nxt){
		...
		*rb = bp = i82563rballoc();
		if(bp == nil) {
			print("%Æ: all %d rx buffers in use, nrbfull %d\n",
				ctlr, Nrb, nrbfull);
			break;
		}
		rd = &ctlr->rdba[rdt];
...
i82563tproc(void *v)
{
	...
	ctlr = edev->ctlr;
	while (!edev->link)
		tsleep(&ctlr->trendez, i82563lim, ctlr, 100);
	ctlr->attticks = sys->ticks;
	for(;;){
		ienable(ctlr, Txdw|Txqe|Txdlow|Parityeccrst);
		/* time out only to diagnose lack of interrupts */
		tsleep(&ctlr->trendez, txrdy, ctlr, 2000);
		ctlr->tim = 0;
		/*
...
eeread(Ctlr *ctlr, int adr)
{
	...
	coherence();
	for (n = 1000*1000; (ctlr->regs[Eerd] & EEdone) == 0 && --n > 0; )
		microdelay(1);
	if (n <= 0)
		iprint("%Æ: eeread stuck for adr %d; continuing...\n", ctlr, adr);
...
eeload(Ctlr *ctlr)
{
	...
	if (ctlr->hasfutextnvm)
		return 0;	/* Eerd doesn't exist; it's now Fextnvm5 */

	sum = 0;
	for (adr = 0; adr < nelem(ctlr->eeprom); adr++) {
...
loadnvrammac(Ctlr *ctlr)
{
	...
	if (ctlr->hasfutextnvm) {		/* no Eec, Eerd registers */
		print("%Æ: has futextnvm, no flash nor eeprom\n", ctlr);
		return 0;
	}
...
//	case i82578:			/* not yet implemented */
	...
	case i217:			/* guessing it might work */
	case i350:			/* guessing it might work */
		r = 0;		/* assume no flash, skip checksum */
		eec = ctlr->regs[Eec];
		if(!(eec & EEautord))
			print("%Æ: ee auto read not done\n", ctlr);
		else if(!(eec & EEpres))
			print("%Æ: eeprom not present\n", ctlr);
		else if(eec & EEflsinuse) {
			print("%Æ: fload due to flash in use\n", ctlr);
			r = fload(ctlr);
		} else if(eec & EEflsdet) {
			print("%Æ: fload due to flash valid\n", ctlr);
			r = fload(ctlr);
		}
		// else print("%Æ: no flash, no eeprom\n", ctlr);
		break;
	default:
		print("%Æ: eeload\n", ctlr);
		r = eeload(ctlr);
...
flowctl(Ctlr *ctlr)
{
	...
	regs[Fcttv] = 0x0100;	/* xmit timer, 64 byte units; for XOFF frame */
	if (!ctlr->hasfutextnvm) {
...
nrcvaddrs(int type)
{
	...
	case i350:
		return 32;
	default:			/* includes ich8 */
		return 7;		/* e.g., 82577. caution; some have 15 */
	}
}
...
setmacs(Ctlr *ctlr)
{
	...
	if (eal == 0 && eah == 0)		/* not yet set in hw? */
		...
		if (r < 0)		/* may have no flash, like the i210 */
			print("%Æ: no mac address found\n", ctlr);
		else {
			/* set mac addr in ctlr from eeprom */
			...
			if (ctlr->type != i217)
				p[Eaddrlen-1] += (regs[Status] & Lanid) >> 2;

			/* set mac addr in regs from ctlr->ra */
			regs[Ral] = eal = p[3]<<24 | p[2]<<16 | p[1]<<8 | p[0];
			regs[Rah] = eah = p[5]<<8 | p[4] | Enable;
			print("%Æ: mac addr %.2lux %.4lux\n", ctlr, eah, eal);
			/*
			 * zero other mac addresses.  can't use memset.
			 * AV (Enable) bits should be zeroed by master reset.
			 */
			naddrs = nrcvaddrs(ctlr->type);
			for(i = 1; i < naddrs; i++)
				regs[Ral+2*i] = regs[Rah+2*i] = 0;
		}
	p = leputl(p, eal);
	*p++ = eah;
	*p = eah >> 8;
...
//		r = 0x10000;	/* works for pxe boot rom; manual suggests 0 */
	...
	regs[Rxdctl] = r;
	if (ctlr->multiq)
		awaitbitpat(&regs[Rxdctl], Qenable, Qenable);
	return r;
}
...
i82563rxinit(Ctlr* ctlr)
{
	...
	type = ctlr->type;
	/*
	 * The i350 has a bug: it always strips the CRC.
	 */
	rctl = Dpf | Bsize2048 | Bam | Rdtms½ | Secrc;	/* no Vfe */
	if(ctlr->multiq){
		/*
		 * Setting Qenable in Rxdctl does not stick unless Ren is on.
		 */
		regs[Rctl] = Ren | rctl;
		awaitbitpat(&regs[Rctl], Ren, Ren);
		regs[Rxdctl] |= Qenable;
		coherence();
	}
	regs[Rctl] = rctl;	/* disable rcvr before we reconfigure it */
	awaitbitpat(&regs[Rctl], Ren, 0);
	...
	case i217:
		...
		break;
	case i350:
		regs[Rfctl] |= Ipv6xsumdis | Lef;	/* erratum #37 + lef */
		regs[Srrctl] = HOWMANY(Rbsz, 1024);
		break;
	}
	if (Touchqs && ctlr->multiq) {
		Perq *rpq;

		/* use only receive queue 0 */
		for (q = 1; q < ctlr->nqueues; q++) {
			rpq = qrxregs(ctlr, q);
			rpq->dbah = rpq->dbal = rpq->dlen = rpq->xdctl = 0;
		}
	}
	coherence();
	...
	/*
	 * Don't enable IP checksum offload.  In practice, it interferes with
...
i82563rproc(void* arg)
{
	...
	ctlr = edev->ctlr;
	while (!edev->link)
		tsleep(&ctlr->rrendez, i82563lim, ctlr, 100);
	ctlr->attticks = sys->ticks;
	for(;;){
		i82563replenish(ctlr);
		/*
		...
		if (ctlr->rintr < 2*Nrd || edev->outpackets > 10)
			rxt0 = Rxt0;
		ctlr->rsleep++;
		ienable(ctlr, Rcvbaseintrs | rxt0 | Parityeccrst);
		...
		for(;;){
			...
			qed = enqinpkt(edev, rd, ctlr->rb[rdh]);
			if (qed < 0)
				break;
			passed += qed;
...
/*
 *  phy, kumeran, etc. rubbish, trash, garbage, swill, tedium, dreck, etc.
 */

static int
phynum(Ctlr *ctlr, int reg)
{
	// TODO: values for i350 are unknown.  one of them appears to be
	// the port number in a multi-port controller.
	// 6 for sgmii rj45?
	if (ctlr->phyaddgen < 0)
		switch (ctlr->type) {
		case i82577:
...
static void
linkup(Ctlr *ctlr, Ether *edev, int sp, int prevlink)
{
	ctlr->speeds[sp]++;
	if (speedtab[sp])
	...
	for(;;){
		...
		if(edev->link){
			if (sp < 1)		/* implausible nowadays */
				sp = 1;
			linkup(ctlr, edev, sp, prevlink);
		} else
			ctlr->didk1fix = 0;	/* force fix at next link up */
		prevlink = edev->link;
next:
		ctlr->lim = 0;
		ienable(ctlr, Lsc|Parityeccrst|Intassert);
		ctlr->lsleep++;
...
i82563attach(Ether* edev)
{
	...
	/* small allocations: descriptors & Block pointers */
	...
	/* bigger allocations: Block receive buffers */
	for(i = 0; i < Nrb; i++){
	...
	initmark(&ctlr->wmtd, (Maxtd < Ntd? Maxtd: Ntd),
		"xmit descriptor queue");
...
i82563interrupt(Ureg*, void* arg)
{
	...
	if (forme) {
		...
		ctlr->im = im = eccerr217(ctlr, icr, im);
	}
	regs[Ims] = im;
	iunlock(&ctlr->imlock);
	if (forme)
	...
	if (0)
		iprint("%Æ: intr not for me, icr %#lux ctlr->im %#ux\n",
			ctlr, icr, ctlr->im);
	return Intrnotforme;
}
...
i82575interrupt(Ureg*, void *)
{
	...
	USED(forme);
#ifndef INTRSVCVOID
	return forme;			/* plan 9 precise accounting */
#endif
}
...
/* interrupt vector allocation, apparently backported from 10GbE controllers */
enum {
	Ivarok	= 1<<7,
	Rxqoff	= 0,
	Txqoff,
...
/* map queue 2*qno+qoff related causes to eicr bit icrbitno (low 8 bits) */
static void
...
ctlrreset(Ctlr *ctlr)
{
	...
	if (ctlr->type == i82566 || ctlr->type == i82567 ||
	    ctlr->type == i82579 || ctlr->type == i350)
		rsts |= Phyrst;
	takeswflag(ctlr);
	ctlr->regs[Ctrl] |= Swrst | rsts;
	coherence();
	delay(20);			/* wait for eeprom reload */
	/* Swflag will be cleared (released) by reset */
	if (awaitbitpat(&ctlr->regs[Ctrl], rsts, 0) < 0) {
		print("%Æ: Ctlr(%#ux).Swrst(%#ux) never went to 0\n",
			ctlr, ctlr->regs[Ctrl], rsts);
		return -1;
	}
	if (rsts & Phyrst)		/* i350 requires this; others too? */
		ctlr->regs[Ctrl] &= ~Phyrst;
	if (ctlr->type == i219)
		ctlr->regs[Fextnvm11] |= F11norsthang;	/* avoid reset hang */
	return 0;
}
...
eerst(Ctlr *ctlr)
{
	...
	if (awaitregbitzero(ctlr, Ctrlext, Eerst) < 0) {
		vmbotch(Parallels, "your actual igbe's Ctrlext's "
...
i82563detach0(Ctlr* ctlr)
{
	...
	takeswflag(ctlr);
	if (ctlr->type == i350)
		regs[Eeer] = 0;		/* disable EEE, per intel */
	regs[Ctrl] |= Slu;
	relswflag(ctlr);
...
i82563reset(Ctlr *ctlr)
{
	...
	if (ctlr->type == i350) {
		/* configure interrupt -> eicr cause bit mapping */
		for (i = 0; i < 4; i++)
			regs[Ivar + i] = 0;
#ifdef USE_EICR
		/* don't need these nor Ei* regs; Icr, etc. suffice */
...
newctlr(Pcidev *p, int type)
{
	...
	/* i218 and i219 have single queues */
	ctlr->multiq = type == i82575 || type == i82576 || type == i82580 ||
		type == i350 || type == i210;
	ctlr->nqueues = ctlr->multiq? 2: 1;	/* default */
	switch (type) {
	case i82576:
	case i210:
		ctlr->nqueues = 4;
	...
	ctlr->hasfutextnvm = ctlr->type == i217 || ctlr->type == i218;
...
i82563pci(void)
{
	...
	while(p = pcimatch(p, Vintel, 0)){
		...
		case 0x150e:		/* 82580EB/DB */
			type = i82580;
			break;
		case 0x151f:		/* i350 no eeprom */
		case 0x1520:		/* i350 virtual */
		case 0x1521:		/* i350 copper */
		case 0x1522:		/* i350 fiber */
		case 0x1523:		/* i350 backplane */
		case 0x1524:		/* i350 ext. sgmii phy */
		case 0x152f:		/* i350 virtual */
			type = i350;
			break;
		case 0x1531:		/* i210 */
		case 0x1532:		/* i211 */
		...
		case 0x1538:		/* i210 sgmii w/ ext. phy */
		case 0x1539:		/* i211 */
		case 0x157b:		/* i210 with invm & without flash */
		case 0x157c:		/* i210 " */
			type = i210;
			break;
		case 0x153a:		/* i217-lm no eeprom */
#ifdef ALLOW_217
			...
			continue;
#endif
		case 0x153b:		/* i217-v no eeprom */
			type = i217;	/* integrated w/ intel 8/c220 chipset */
			break;
		case 0x1559:		/* i218-v */
		case 0x155a:		/* i218-lm (fancier model) */
		case 0x15a0:		/* i218-lm */
		case 0x15a1:		/* i218-v */
		case 0x15a2:		/* i218-lm */
		case 0x15a3:		/* i218-v */
			type = i218;
			break;
		case 0x156f:		/* i219-lm (fancier model) */
		case 0x1570:		/* i219-v */
		case 0x1589:		/* i219-lm */
		case 0x15b7:		/* i219-lm */
		case 0x15b8:		/* i219-v */
...
static int i350pnp(Ether *e) { return pnp(e, i350); }
