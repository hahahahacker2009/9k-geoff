...
/*
 * return cpu's apicno and set v->cpu.
 * cpu0 is assumed to be always up and usable.
 * we need xapic for this cpu already set up.
 */
static int
usecpu(Vctl *v, uint cpu)
{
	...
	for (apicno = 0; apicno <= Napic; apicno++) {
		...
		if (apicavail(apic)) {
			v->cpu = cpu;
			if (0)
				iprint("usecpu: cpu%d is apic %d\n",
					cpu, apicno);
			return apicno;
		} else {
			DBG("usecpu: cpu%d: useable %d addr %#p\n",
				cpu, apic->useable, apic->addr);
		}
	}
	if (cpu0apicno >= 0) {
		/* doesn't suffice if we have hyperthreading on */
		v->cpu = 0;
		return cpu0apicno;
	}
	panic("usecpu: cpu%d: no useable apic found", cpu);
}
...
/*
 * Set delivery mode (lo) and destination field (hi), including cpu choice,
...
ioapicintrdd(u32int* hi, u32int* lo, Vctl *v)
{
	...
	*lo |= Pm | MTf;
	coherence();
	// print("ioapicintrdd hi %#ux lo %#ux\n", *hi, *lo);
}
