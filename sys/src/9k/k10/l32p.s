...
TEXT _multibootheader<>(SB), 1, $-4		/* must be 4-byte aligned */
	...
	LONG	$-(MBOOTHDRMAG + 0x00000003)	/* checksum */
...
DBGPUT('L', loopL)
	...
	MOVQ	SI, SP
	ADDQ	$(MACHSTKSZ-8), SP	/* establish stack in sys */
	...
	MOVQ	SI, DI
	ADDQ	$(MACHSTKSZ + 4*PTSZ), DI /* skip stack & page tables */
	MOVQ	$((MACHSZ + 3*PGSZ)>>2), CX
...
/*
 * Make the basic page tables for CPU0 to map 0-8MB (was 0-4MB) physical
 ...
 */
/*
 * The layout is described in dat.h as Sys, in reverse order by address,
 * below the kernel and above the low 1MB:
 *
...
TEXT _warp64<>(SB), 1, $-4
	...
	MOVL	$(8*MB-8), SP		/* establish temporary stack */
DBGPUT('z', zloop)
	MOVL	SI, DI
	XORL	AX, AX
...
POPL	AX
	 ...
	 * we don't use the first MB, and 9k10fs is ~3MB.
	 */
	LEAL	LOWPD(SI), AX
 ...
 */
	MOVQ	AX, sys(SB)			/* set sys for C and below */
