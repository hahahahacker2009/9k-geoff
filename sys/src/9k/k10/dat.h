 ...
 */
struct MCPU {
	/* CPUID instr. output E[ABCD]X for Highstdfunc & Procsig */
	u32int	cpuinfo[2][4];		/* CPUID instr. output */
	int	ncpuinfos;		/* number of standard entries */
	int	ncpuinfoe;		/* number of extended entries */
	uchar	isintelcpu;		/* flag */
	uchar	intelcputype;
};
...
/*
 * This is the low memory map, between 0x100000 and 0x120000 (128K @ 1MB).
 ...
 * sipi().  The sipi code assumes pml4 follows machstk and that the address
 * of either can be derived from the other, so keep them together and in order.
 * Currently 48K.
 */
struct Syspercpu {
	uchar	machstk[MACHSTKSZ];	/* must be multiple of PGSZ */

	PTE	pml4[PTSZ/sizeof(PTE)];	/* initial page tables */
	PTE	pdp[PTSZ/sizeof(PTE)];	/* aka PUD or PDPT */
	PTE	pd[PTSZ/sizeof(PTE)];	/* aka PMD */
...
//			uvlong	tscoff[MACHMAX]; /* diff from cpu0's tsc */
	...
	/*
	 * there is supposed to be a performance advantage to putting machptr
	 * in its own page on x86.  either hardware or VMs run faster this way.
	 */
	union {
...
enum {				/* cpuid standard function codes */
	...
	Procid7 = 7,
	Proctype = 0x1a,	/* intel: ax upper byte: 0x20 atom, 0x40 xeon */
	...
	Extbrand1,
	Extbrand2,
	Extmiscfeat = Highextfunc + 7,	/* miscellaneous features */
	Extcap = Highextfunc + 8,	/* address sizes */
};
...
#define DBGFLG		0
#define DBG(...)	if(!DBGFLG) {} else iprint(__VA_ARGS__)
...
enum {
	...
	Tsc	= 1<<4,		/* time-stamp counter (1993) */
	Cpumsr	= 1<<5,		/* model-specific reg.s, rdmsr/wrmsr (1993) */
	Paep	= 1<<6,		/* physical-addr extensions */
	Mcep	= 1<<7,		/* machine-check exception */
	Cmpxchg8b = 1<<8,
	Cpuapic	= 1<<9,		/* have local apic (1992, 1994 mandatory) */
	Mtrr	= 1<<12,	/* memory-type range regs. */
	Pgep	= 1<<13,	/* page global extension */
	Pse2	= 1<<17,	/* more page size extensions */
	Clflush = 1<<19,	/* have the instruction */
	Mmx	= 1<<23,
	...
	L5pt	= 1<<16,	/* supports 5-level page tables, 57-bit addrs */

	/* Procid7 BX */
	Clflushopt = 1<<23,	/* have instruction */
	/*
	 * have cache line write back; new in 2014 (2019 on amd).
	 * intended for use with optane or other non-volatile ram.
	 */
	Clwb	= 1<<24,
	...
	/* Extmiscfeat DX */
	Invartsc = 1<<8,	/* time-stamp counter is invariant */
};
...
enum {				/* Msrperfctl bits */
	Turbooff = 1ll<<32,	/* turbo boost disabled */
};
...
enum {
	Wdogms = 100,			/* interval to restart 1 s. watchdog */
};
