...
/*
 * names _intrp and _intre are known to ktrace.
 *
 * the fault machinery pushes [ss, sp,] flags, cs, interrupted ip, and sometimes
 * an error code, before calling into idthandlers, which calls these (_intre
 * when there is an error code) with a CALL instruction, which pushes its return
 * PC.  IRETQ restores what the fault machinery pushes, other than error code.
 */
TEXT _intrp<>(SB), 1, $-4			/* no error code pushed */
	PUSHQ	AX				/* save AX below CALL ret PC */
	MOVQ	8(SP), AX			/* idthandlers(SB) PC */
	JMP	_intrcommon

TEXT _intre<>(SB), 1, $-4			/* error code pushed by cpu */
	XCHGQ	AX, (SP)			/* save AX, load CALL ret PC */
_intrcommon:
	MOVBQZX	(AX), AX		/* byte after CALL: fault # */
	XCHGQ	AX, (SP)		/* fault # to stack, restore AX */
...
_iretnested:
	ADDQ	$40, SP		/* also skips (pc or error) and fault # */
	IRETQ
...
//	CALL _intrp<>(SB); BYTE $0x3e	/* IdtIPI (62) real mode */
MODE $16
