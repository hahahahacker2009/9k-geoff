 ...
 *	  R8 or above in real mode.  R14 and R15 are up and m, so safe.
 */
TEXT realmode0(SB), $0
//	CALL	saveregs(SB)			/* TODO: remove */
	/* switch to low code address, assumed mapped */
...
TEXT realmodeintrinst(SB), $0
	...
	/* re-enter protected mode and jump to 64-bit code */
	OPSIZE; MOVL	CR0, AX
	OPSIZE; ORL	$Pe, AX
	OPSIZE; MOVL	AX, CR0
...
//	CMPL	AX, AX		// TODO
//	JEQ	_skippushs
	PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX
	PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX
	PUSHQ AX; PUSHQ AX; PUSHQ AX; PUSHQ AX
	ADDQ	$(SAVEDREGS*BY2V), SP
_skippushs:
	/*
	 * swap return PC and saved flags.  (16*BY2V)(SP) contains saved flags
	 * due to swap in saveregs, 0(SP) holds saved AX (return PC).
	 */
	XCHGQ	(SAVEDREGS*BY2V)(SP), AX
	XCHGQ	0(SP), AX
	XCHGQ	(SAVEDREGS*BY2V)(SP), AX
...
//	CMPL	AX, AX		// TODO
//	JEQ	_skippops
	SUBQ	$(SAVEDREGS*BY2V), SP
	POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX
	POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX; POPQ AX
_skippops:
	/* surely we should push ax before popping into it above */
	PUSHQ	AX		/* TODO see l64idt.s */
	PUSHQ	BX
	PUSHQ	CX
	PUSHQ	DX
	PUSHQ	BP
	...
	/*
	 * swap return PC and saved flags.  (16*BY2V)(SP) contains saved AX
	 * (return PC), 0(SP) holds saved flags.
	 */
	XCHGQ	(SAVEDREGS*BY2V)(SP), AX
	XCHGQ	0(SP), AX
	XCHGQ	(SAVEDREGS*BY2V)(SP), AX
