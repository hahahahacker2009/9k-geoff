...
/* 66 0f ae 3x	CLWB (r); x = 0 for ax, 1 for cx, 2 for dx, 3 for bx */
/* CLWB (BP) */
#define CLWB	BYTE $0x66; BYTE $0x0f; BYTE $0xae; BYTE $0x75; BYTE $0x00

MODE $64

/*
 * Port I/O.  Welcome to 1972 (the 8008), at the latest; probably actually
 * to 1948.  In 1970, the PDP-11 provided memory-mapped I/O registers, even
 * for DMA, instead of special I/O instructions.  Intel was slow on the
 * uptake, unsurprisingly.
 *
...
_swap:
	...
	MOVLQZX	DX, DX				/* paranoia: zero high bits */
	SHLQ	$32, AX				/* hi<<32 */
	ORQ	DX, AX				/* (hi<<32)|lo */
	RET
...
_spllo:
	...
	MFENCE
	/*
	 * strictly speaking, STI should be after zeroing SPLPC,
	 * but since even the profiling clock can't interrupt splhi,
	 * we get more meaningful profiling results with STI before it.
	 */
	STI					/* enable intrs; expect intr. */
	MOVQ	$0, 8(RMACH)			/* enabling; clear m->splpc */
	RET
...
/*
 * atomic operations
 * destination operands should be naturally aligned, in case of cpu errata
 */
