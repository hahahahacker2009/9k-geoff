...
enum
{
	Giveadamn	= 1,		/* print errors, even on ga-6bxd */
	...
	Mie =	1 << 31,
	Oc =	1 << 30,
	Rhsc =	1 << 6,			/* root hub status changed */
	Fno =	1 << 5,
	Ue =	1 << 4,
	Rd =	1 << 3,
	Sf =	1 << 2,
 ...
 */
struct Ed {
	/* these are physical addresses, so must be in lower 4 GB */
	ulong	ctrl;
...
tdalloc(void)
{
	...
	if(tdpool.free == nil){
		...
		pool = mallocalign(Incr*sizeof(Td), Align, 0, 0);
		if(pool == nil)
			panic("tdalloc");
		for(i=Incr; --i>=0;){
...
interrupt(Ureg *, void *arg)
{
	...
	if(status & Rhsc){
		if(Giveadamn)
			print("ohci: root hub status changed\n");
		status &= ~Rhsc;
	}
	if((status & Ue) != 0){
...
ohcireset(Ctlr *ctlr)
{
	...
	/* legacy support (bios handoff) register: turn off lunacy mode */
