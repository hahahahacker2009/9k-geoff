...
#define MACHSTKSZ	(6*PGSZ)		/* Mach stack size; pages */
/*
 ...
 * registers or magic untouchable ACPI memory.  Note that l32p.s knows this
 * size.
 */
...
#define KSEG2		(VZERO-ADDRSPCSZ)	/* up to KMAP (PMAPADDR) */

/*
 * KZERO is the kernel virtual address that maps to physical 0 and the kernel
 * typically begins a little above KZERO.  (KSEG2 may be a lower kernel virtual
 * address.)  The kernel's text, data and bss segments must fit between KZERO
 * and the top of kernel address space (VZERO).  KZERO must be a contiguous
 * mask in the highest-order address bits.  In practice, it's always KSEG0,
 * and it's not clear why it would ever not be.
 */
#define KZERO		KSEG0
#define KTZERO		(KZERO+1*MB+128*KB)	/* must match mkfile 6l -T */
...
#define PTLX(v, l)	(((v) >> (((l)*PTSHFT)+PGSHFT)) & ((1ull<<PTSHFT)-1))
...
/*
 * we need the physical addr. of lapic base in the msi addr. reg.
 */
