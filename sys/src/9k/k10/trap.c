...
trapunassigned(Ureg *ureg, int vno)
{
	...
	if (!active.exiting && !active.rebooting) {
		...
		if (!thisisapu2 || vno != IdtPIC+7 || traps > sys->nonline)
			if (vno < 32)		/* processor trap? */
				panic("cpu%d: trap for unassigned vector %d "
					"(%#x) @ %#p%s", m->machno, vno, vno,
					ureg->ip, disabled? ", disabled": "");
			else
				iprint("cpu%d: intr for unassigned vector %d "
					"(%#x) @ %#p%s\n", m->machno, vno, vno,
					ureg->ip, disabled? ", disabled": "");
	}
	apiceoi(vno);
...
gpf(Ureg* ureg, void*)
{
	 ...
	 * tries to validate pc so a wild jump doesn't cause recursive GPFs.
	 */
	inst = (uchar *)ureg->pc;
	user = userureg(ureg);
	if (!user && inst >= (uchar *)_main && inst < (uchar *)etext &&
	    inst[0] == Msr0 && (inst[1] == Rdmsr1 || inst[1] == Wrmsr1)) {
