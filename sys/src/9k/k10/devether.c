...
etherconffmt(char *p, char *e, Ether *ether, int cardno)
{
	...
	if (ether->pcidev)
		p = seprint(p, e, "vid %#ux did %#ux ",
			ether->pcidev->vid, ether->pcidev->did);
	if(ether->mbps >= 1000)
...
etherfmt(Fmt* fmt)
{
	...
	case L'æ':				/* compatibility with old */
		...
		/* Ethident must be first member of Ctlr */
		ethid = (Ethident *)ctlr;
		if (ethid->edev == nil)
			snprint(p, Ethidlen, "#l?: %s", ethid->prtype);
		else
			snprint(p, Ethidlen, "#l%d: %s",
				ethid->edev->ctlrno, ethid->prtype);
		break;
	default:
