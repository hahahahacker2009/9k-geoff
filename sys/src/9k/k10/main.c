...
enum {
	...
	Minmb = 128,
	Kernfract = 32,		/* fraction of mem for kernel; enough for Pages */
};
...
Sys* sys = nil;				/* set in l32p.s */
uintptr sizeofSys = sizeof(Sys);
uintptr sizeofSyspercpu = sizeof(Syspercpu);

/*
 * Option arguments nominally from the non-existent command line.
 * oargv[0] is the boot file.
 * mboptinit() is called from multiboot() to set it all up.
 */
static int oargc;
...
/* some amd64 bioses still set mtrrs wrong or reordered, notably apu2's */
static void
checkmtrr(void)
{
	int i, vcnt;
	 ...
	 */
	if(!(m->cpuinfo[Procsig][Dx] & Mtrr))
		return;

	m->mtrrcap = rdmsr(MTRRCap);
	m->mtrrdef = rdmsr(MTRRDefaultType);
	if(m->mtrrcap & Capfix){
		m->mtrrfix[0] = rdmsr(Msrmtrrfix0);
		m->mtrrfix[1] = rdmsr(Msrmtrrfix1);
		m->mtrrfix[2] = rdmsr(Msrmtrrfix2);
		for(i = 0; i < 8; i++)
			m->mtrrfix[(i+3)] = rdmsr(Msrmtrrfix3+i);
	}
	...
	for(i = 0; i < vcnt; i++)
		...
		m->mtrrvar[i] = rdmsr(MTRRPhysBase0+2*i);	
...
/*
 * start a cpu other than 0, at apicno.  entered splhi from l16sipi.s copied to
 * SIPIHANDLER from sipihandler, with mmu on and in long mode.
 */
void
squidboy(int apicno)
{
	...
	fpuinit();
	checkmtrr();		/* compare with cpu0's mtrrs; should sync */
...
//	acpiinit();			/* no-op was generated by ../mk/parse */
	mpsinit();
	/*
	 * the first apic appeared in 1992 and became integrated with the intel
	 * p54c in 1994.  timestamp counter and rdmsr and wrmsr instructions
	 * appeared in the pentium in 1993.
	 */
	cpuidinfo(Procsig, 0, info);
	if((info[Dx] & (Cpuapic|Cpumsr|Tsc)) != (Cpuapic|Cpumsr|Tsc))
		panic("antique cpu is pre-amd64: lacks apics, msrs or tsc; "
			"cpuiddx %#ux", info[Dx]);
...
main(u32int mbmagic, u32int mbi)
{
	...
	print("\nPlan 9 (amd64)\n\n");
	sethz();		/* prints cpu info; may use timer I/O ports */
	sanity();
	cpuidcaps();
	if (isapu())
