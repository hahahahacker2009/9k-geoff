...
apicavail(Apic *apic)
{
	return apic->useable && apic->addr == nil;
}
...
apicinit(int apicno, uintmem pa, int isbp)
{
	 ...
	 * to be dealt with sometime (or ignored, more likely).
	 */
...
apictmrfreq(Apic *apic)
{
	...
	s = splhi();  /* should be high, but interrupts would disrupt timing */
	apicrput(Tdc, DivX1);
...
/* force lapic out of x2apic mode if needed, then enable */
static void
lapiccfg(void)
{
	vlong base;

	base = rdmsr(Msrapicbase);
	if ((base & (Lapicae|Lapicx2)) != Lapicae) {
		/*
		 * if enabled in x2apic mode, can't just switch to enabled
		 * non-x2apic mode: it will GPF.
		 */
		iprint("reconfiguring lapic\n");
		if ((base & (Lapicae|Lapicx2)) == (Lapicae|Lapicx2)) {
			base &= ~(uvlong)(Lapicae|Lapicx2);	/* disable */
			wrmsr(Msrapicbase, base);
		}
		wrmsr(Msrapicbase, base | Lapicae);	/* enable */
	}
}
...
apiconline(void)
{
	 ...
	 */
	if(memcmp(&m->cpuinfo[Highstdfunc][Bx], "AuthenticAMD", 12) == 0)
		dfr = 0xf0000000;
	else
		dfr = ~0ul;
	apicrput(Df, dfr);
...
apicsipi(int apicno, uintptr pa)
{
	...
	DBG("apic%d: ipis: init...", apicno);	// TODO
	apicinitipi(apicno);
...
lapicspurious(Ureg*, void*)
{
	...
	if (1 || (aadd(&traps, 1) > sys->nonline || sys->ticks > 60*HZ) &&
	    traps < 3*sys->nonline)	// TODO: spurious lapic intrs
