...
static Perq *qrxregs(Ctlr *ctlr, uint n);
static Perq *qtxregs(Ctlr *ctlr, uint n);
...
//	r = Gran;			/* works for pxe boot rom */
	r = Gran | 1<<HthreshSHIFT | 1<<LwthreshSHIFT |
		1<<WthreshSHIFT | 1<<PthreshSHIFT;
	if (Touchqs)
		/* use only transmit queue 0 */
		for (q = 1; q < ctlr->nqueues; q++) {
			tpq = qtxregs(ctlr, q);
			tpq->dbah = tpq->dbal = tpq->dlen = 0;
			tpq->xdctl = r;		/* erratum workaround */
		}
	coherence();
	return r;
}
...
eccerr217(Ctlr *ctlr, ulong icr, ulong im)
{
	...
	if (ctlr->type == i217 && icr & Parityeccrst) {
		if (oddities++ == 0)
