...
enum
{
	...
	MaxMTU=	IP_MAX,			/* allow for jumbo frames & large UDP */
...
static void	bretherwrite(Port *port, Block *bp);
...
{
	ctl->dev->write(ctl, s, strlen(s), 0);
}
...
portbind(Bridge *b, int argc, char *argv[])
{
	...
	if(type == Tether) {
		if(argc != 4)
			error(usage);
	} else {		/* tunnel: dev is tunnel, dev2 is ether */
		if(argc != 5)
			error(usage);
		dev2 = argv[4];
	}
	...
	if (port != nil) {
		if (Debug)
			print("bridge port for (%s, %s) in use\n",
				name, argv[0]);
	...
	switch(port->type) {
	default:
		panic("devbridge portbind: unknown port type: %d", type);
	case Tether:
		...
		// get directory name
		n = ctl->dev->read(ctl, buf, sizeof(buf)-1, 0);
		buf[sizeof(buf)-1] = '\0';
		buf[n >= 0? n: 0] = 0;
...
cachelookup(Bridge *b, uchar d[Eaddrlen])
{
	...
	p = b->cache + cachehash(d, Eaddrlen);
...
cacheupdate(Bridge *b, uchar d[Eaddrlen], int port)
{
	...
	pp = p = b->cache + cachehash(d, Eaddrlen);
	sec = p->expire;
...
static void
brethermultiwrite(Bridge *b, Block *bp, Port *port)
{
	Port *oport;
	Etherpkt *ep;
	int i, mcast;
	...
	for(i=0; i<b->nport; i++) {
		...
		if(oport != nil) {
			b->copy++;
			bretherwrite(oport, copyblock(bp, blocklen(bp)));
		}
		oport = b->port[i];
	}
...
bretherread(void *a)
{
	...
	while(!port->closed){
		...
		if(0)
			print("bretherread: reading\n");
		/* we'd need to add some slop to ETHERMAXTU to support VLANs */
		bp = pdc->dev->bread(pdc, MaxMTU, 0);
		poperror();

		qlock(b);
		/* bp may be just the first Block of the packet */
		if(bp == nil)
			break;
		n = blocklen(bp);
		if(0)
			print("bretherread: blocklen = %ld\n", n);
		...
		if(ep->d[0] & 1) {
			...
			port->inmulti++;
			/* etheroq will eventually free bp */
			brethermultiwrite(b, bp, port);
		} else {
			...
			if(ce == nil) {
				...
				b->hit++;
				bretherwrite(b->port[ce->port], bp);
			} else
				freeb(bp);
		}
	}
...
static void
bretherwrite(Port *port, Block *bp)
{
	Chan *pdd;
	Ip4hdr *eh, *feh;
	Ip4hinfo h4info;
	...
	if(port->type != Ttun || !fragment(epkt, n)) {
		 ...
		 * just write it and go home.  common case.
		 */
		if(!waserror()){
			pdd->dev->bwrite(pdd, bp, 0);
			poperror();
		}
		return;
	}
	...
	if(waserror()){
		freeblist(bp);
		return;		/* was nexterror(), which seems plausible */
	}

	switch(nhgets(epkt->type)){
	default:
		error("bretherwrite cannot fragment non-IPv4 packet");
	case ETIP4:
		break;
	}
