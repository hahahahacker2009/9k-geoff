...
checkhdr(Execdata *ed, Hdr *hdrp)
{
	...
	if(ed->hdrsz != sizeof(Hdr) ||
	    (ed->magic & ~HDR_MAGIC) != (AOUT_MAGIC & ~HDR_MAGIC))
	...
	ed->textmin = PGROUND(UTZERO + ed->hdrsz + ed->textsz);
	ed->textlim = UTROUND(ed->textmin);
	ed->datalim = PGROUND(ed->textlim + ed->datasz);
	ed->bsslim =  PGROUND(ed->textlim + ed->datasz + ed->bsssz);
...
newstack(Execdata *ed)
{
	...
	tos = (Tos*)ed->stack;
	tos->cyclefreq = m->cpuhz;	/* we often get a page fault here. */
...
copystack(Execdata *ed, char *args, va_list *listp)
{
	 ...
	 */
	if(nargs <= 0)
		error("exec arguments length non-positive");
...
sysnoted(Ar0* ar0, va_list list)
{
	...
	if(v != NRSTR && !up->notified)
		error("noted: not restore and not notified");
...
/* Add new semaphore p with addr a to list in seg.  zero *p. */
static void
