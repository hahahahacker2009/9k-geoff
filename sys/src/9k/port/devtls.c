...
enum {
	...
	/* buffer limits */
	/* MaxRecLen of 2¹⁴ is required by the TLS spec. */
...
tlsopen(Chan *c, int omode)
{
	...
	c->offset = 0;
	/*
	 * c->iounit used to be set to qiomaxatomic, but these days that
	 * causes mouse motion with galaxy running to kill the cpu connection.
	 * Setting it to MaxRecLen avoids that bug.
	 */
	c->iounit = MaxRecLen;
	return c;
}
