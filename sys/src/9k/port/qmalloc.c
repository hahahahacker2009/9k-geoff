 ...
 *
 * allocates from KZERO space (sys->vmunused to sys->vmend).
 * allocations are double-aligned.
 *
...
static	void	qfreeinternal(void*, uintptr);
...
/* called when we can't recover from lack of free space */
static void
bailout(char *s, uvlong bytes)
{
	mallocsummary();
	panic("%s: out of memory allocating %lld bytes for pid %d %s",
		s, bytes, up? up->pid: 0, up? up->text: "");
}
...
static void
ckcorrupt(char *func, Header *p, uintptr caller)
{
	if(p->s.size == 0)
		panic("%s: corrupt allocation arena: p->s.size 0, p %#p, pc %#p",
			func, p, caller);
	if(p->s.next != &checkval)
		panic("%s: corrupt allocation arena: bad p->s.next %#p, p %#p, pc %#p",
			func, p->s.next, p, caller);
}

static void
qfreeinternal(void* ap, uintptr caller)
{
	Qlist *qlist;
	Header *p, *q;
	uintptr nunits;
	...
	p = (Header*)ap - 1;
	ckcorrupt("qfreeinternal", p, caller);
	nunits = p->s.size;
...
/* print a summary of malloc usage */
void
mallocsummary(void)
{
	Header *q;
...
{
	MLOCK;
	qfreeinternal(ap, getcallerpc(&ap));
	MUNLOCK;
}
...
malloc(uintptr size)
{
	return mallocz(size, 1);	/* kernel allocations always zero */
}
...
mallocalign(uintptr nbytes, uintptr align, vlong offset, uintptr span)
{
	...
	/*
	 * Should this zero or should it be left to the caller?
	 * kernel allocation always zeroes, and the kernel relies on that.
	 */
...
/*
 * malloc, always returns non-nil.  on failure, if we're in a user proc, sleep
 * and retry, else panic.  thus, not to be called while holding a lock.
 */
void*
...
realloc(void* ap, uintptr size)
{
	...
	p = (Header*)ap - 1;
	ckcorrupt("realloc", p, getcallerpc(&ap));
	ounits = p->s.size;
...
mallocinit(void)
{
	if(tailptr != nil || mallocok)
		return;

	tailptr = tailbase = (Header *)sys->vmunused;
	availnunits = HOWMANY(sys->vmend - sys->vmunused, Unitsz);
 if(0)	print("malloc: base %#p vmend %#p vmunused %#p nunits %llud\n",
		tailbase, sys->vmend, sys->vmunused, (uvlong)availnunits);
	mallocok = 1;
}
