...
#define CNTRL(let)	(int)((let) & MASK(5))
...
prflush(void)
{
	...
	for (now = m->ticks; consactive() && m->ticks - now < 10*HZ; )
		pause();
}
...
print(char *fmt, ...)
{
	...
	va_start(arg, fmt);
	if (early) {		/* don't call vseprint from low memory */
		n = vprf(fmt, arg);
		va_end(arg);
		return n;;
	}
...
iprint(char *fmt, ...)
{
	...
	if(locked)
		unlock(&iprintlock);
	splx(s);
	delay(prmsdelay);		/* mainly risc-v debugging */
	return n;
}
...
_panic(char *fmt, ...)
{
	...
	watchdogoff();		/* simplify life by shutting off any watchdog */

	s = splhi();
	strcpy(buf, "panic: ");
	va_start(arg, fmt);
	n = vseprint(buf+strlen(buf), buf+sizeof(buf)-2, fmt, arg) - buf;
	va_end(arg);
	/* fmt might already have trailing newline, but it shouldn't. */
	if (strchr(buf, '\n') == nil)
		strcat(buf, "\n");

	iprint("%s", buf);		/* force panic msg out */
	delay(100);
	chknested();
	if(consdebug)
		(*consdebug)();
...
enum{
	...
	Qkmesg,
	Qkprint,
	Qhostdomain,			/* unused since 9P2000 */
	Qhostowner,
	Qnull,
	Qosversion,
	Qpgrpid,
...
consread(Chan *c, void *buf, long n, vlong off)
{
	...
	case Qkmesg:
		...
		if(off >= kmesg.n)
			...
			/* replace NULs that sneak in */
			for (b = buf; b < (char *)buf + n; b++)
				if (*b == '\0')
					*b = ' ';
		}
		return n;
	...
	case Qswap:
		l = snprint(tmp, sizeof tmp,
			...
			sys->pmoccupied,
			PGSZ,
			(vlong)PGROUND(sys->vmend - KTZERO)/PGSZ,
	...
	default:
		print("consread %#llux\n", c->qid.path);
		error("consread of unknown file");
		notreached();
	}
}
...
conswrite(Chan *c, void *va, long n, vlong off)
{
	...
	default:
		...
		error("conswrite of unknown file");
	}
	return n;
}
