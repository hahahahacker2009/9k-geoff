...
traceret(Ar0 *ar0p, uintptr sp, int scallnr, vlong startns)
{
	...
	sysretfmt(scallnr, (va_list)SKIPSTKPC(sp), ar0p, startns, todget(nil));
	stopme_freetrace();
}
...
static void
dosyscall(uint scallnr, Ureg* ureg, Ar0 *ar0p, uintptr sp)
{
	Systab *scall;
...
syscall(uint scallnr, Ureg* ureg, Ar0 *ar0p)
{
	...
	spllo();
	dosyscall(scallnr, ureg, ar0p, sp);
	...
	kexit(ureg);			/* likely to trigger a page fault */
}
