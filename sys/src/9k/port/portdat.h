...
struct Block
{
	...
	long	ref; /* unused in 9k, needed to compile; just for 9/ip/gre.c */
};
...
struct Page
{
	...
	/* Disc address in file, or # of user mappings or index in top-level PT */
	ulong	daddr;
...
struct Segment
{
	Ref;
	QLock	lk;
	ushort	steal;		/* Page stealer lock for 9's pageout(); obs. */
...
/*
 * almost all return values go into the return register (e.g., ureg->ret or
 * ureg->arg) but nsec's vlong goes into the next register too, on 32-bit
 * systems.  on at least riscv64, we also have to sign-extend the return
 * register.
 */
...
union Ar0 {			/* possible types of syscall return values */
	...
	int	i;
	long	l;
	uintptr	u;		/* uintptr doesn't allow -1 on error */
#endif
	uintptr	p;
	void*	v;
	vlong	vl;
...
struct Proc
{
	...
	int	parentpid;
	ulong	time[6];	/* TUser, TSys, TReal; child U, S, R */
	...
	int	insyscall;	/* flag; also subscript of w.time[] */
	 ...
	 */
	PFPU;
	PFPU notefpu;		/* holds FP regs during handler execution */
	PMMU;
	PNOTIFY;
	double	_align1;
};
...
extern 	Systab	systab[];
	...
	char	cputype[];
	int	dmaincoherent;		/* flag: cheap and sleazy hardware? */
	int	early;	/* flag: not ready for traps yet, also print w/ prf */
	uintptr	pgmem;
	int	polledprint;
	void	(*prfaults)(void);
	int	prmsdelay;
