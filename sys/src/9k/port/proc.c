...
//	if (!m->clockintrsok)			/* TODO?  too early? */
//		panic("sched called when clock intrs not ok (too early?)");
	if(m->ilockdepth)
	...
	if(up){
		 ...
		 */
		coherence();	/* make changes from other cpus visible */
...
ready(Proc *p)
{
	...
	if(edfready(p)){
		splx(s);
		/* calling idlewake here costs system & real time */
		return;
	}
...
/*
 *  sleep if a condition, (*f)(arg), is not true.  Another process will
...
stackuse(char *stkbot, char *stktop)
{
	...
	bot = (uvlong *)ROUNDUP((uintptr)stkbot, BY2V);
	for (; bot < (uvlong *)stktop && *bot == 0; ++bot)
		;
	/* bot is lowest uvlong used in the stack */
...
procflushseg(Segment *s)
{
	...
	/* alternate scheme: remember this cpu's # or m, scan the others */
again:
	/* could be shutting down, so use MACHMAX not sys->nonline */
	for(i = 0; i < MACHMAX; i++)
		...
		/*
		 * we've reverted to essentially the 9 code, because previous
		 * 9k code did this, but sched may change cpu, thus m, so
		 * could miss a cpu because it doesn't start over at again:
		 *	while(mp->mmuflush)
		 *		sched();
		 */
}
...
accounttime(void)
{
	...
	m->perf.avg_inidle = ((uvlong)m->perf.avg_inidle*(HZ-1)+m->perf.inidle)
		/ HZ;
	m->perf.inidle = 0;

	m->perf.avg_inintr = ((uvlong)m->perf.avg_inintr*(HZ-1)+m->perf.inintr)
		/ HZ;
	m->perf.inintr = 0;
