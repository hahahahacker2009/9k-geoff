...
enum
{
	/* must be enough for gs's OFILES in mkfile; 16K used to be enough */
...
/* there are 2 use cases: qidpath==-1 looks up by name, name==nil by qidpath */
static Evalue*
envlookup(Egrp *eg, char *name, ulong qidpath)
{
	...
	eend = &eg->ent[eg->nent];
	if (name == nil) {
		for(ecur = eg->ent; ecur < eend; ecur++)
			if((*ecur)->qid.path == qidpath)
				return *ecur;
	} else {
		name0 = name[0];
		for(ecur = eg->ent; ecur < eend; ecur++) {
			e = *ecur;
			if((e->name[0]==name0 && strcmp(e->name, name) == 0))
				return e;
		}
	}
	return nil;
