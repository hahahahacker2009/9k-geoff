 ...
 * dedicated sequential read performance.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
#define QFULL(qp)	((qp)->sidx.hd == sqidxinc((qp), (qp)->sidx.tl))
#define QEMPTY(qp)	((qp)->sidx.hd == (qp)->sidx.tl)

#define nvmeadmissue(ctlr, op, nsid, buf) \
	nvmeissue(ctlr, &ctlr->qpair[Qadmin], nil, op, nsid, buf, 0, 1)
...
enum {
	...
	Minsect	= 512,
	Iodirs = Write+1+1,	/* read, write, rdwr */
	...
	/*
	 * the kernel may print with interrupts masked for some seconds,
	 * so allow for that.
	 */
	Timeout = 2*60*1000,	/* adjust to taste. started at 2000 ms. */
	...
	/*
	 * busy drive has operations timeout; trying to locate the fault.
	 * queue length of 1 wedges.  2 is fine.
	 */
	Sqlen	= 32,	/* maxima; queue lengths must be powers of 2 < 4K */
	Cqlen	= 16,
...
struct Ctlr {
	...
	short	sqlen;		/* submission q length */
	short	cqlen;		/* completion q length */
	...
	Lock	issuelock;	/* intr svc, regs, inflight & q heads & tail */
	Lock	xfrlock;
	Lock	shutlock;
	...
	short	maxqlen[2];
	uvlong	count[Iodirs];	/* xfers: read, write, rdwr */
	uvlong	totus[Iodirs];	/* sum of (µs per xfer): " */
	uvlong	implaus[Iodirs]; /* implausible cycle counts: " */
	uvlong	overlaps;
	uvlong	drainsleeps;

	/* per-drive bigger arrays */
	char	serial[20+1];
	char	model[40+1];
	char	fw[8+1];
};
...
struct Cmd {			/* submission queue entry (hw) */
...
struct Completion {		/* completion queue entry (hw) */
	ulong	specific;
	ulong	_pad;
	ushort	sqhd;		/* updated by hw, i hope */
	ushort	sqid;
	ushort	cmdid;
...
static void
isfatal(Ctlr *ctlr, char *where)
{
	if (ctlr->regs->csts & Cfs)
		iprint("sd%C0: %s: fatal nvme controller error\n",
			ctlr->sdev->idno, where);
}
...
completexfr(Ctlr *ctlr, Completion *cqhd, int qid)
{
	...
	/* seems to aggravate matters when a sata disk goes offline */
	if (TODO && xfr->qtm && TK2MS(sys->ticks) - xfr->qtm >= Timeout)
	...
	if (Measure) {
		...
		/*
		 * if xfr->stcyc was set on another cpu, with a slightly
		 * staggered cycle counter, cycs may be wrong, including
		 * negative or too big.
		 */
		cycles(&cycs);
		cycs -= xfr->stcyc;
		rdwr = xfr->rdwr;
		if (rdwr < Iodirs)
			if (xfr->machno == m->machno && cycs < m->cpuhz) {
...
advancesqhd(Ctlr *ctlr, Qpair *qp, Completion *cqhd, int qid)
{
	...
	/* could zero slbas from qp->sidx.hd to before cqhd->sqhd */
	qp->sidx.hd = cqhd->sqhd;
...
static void
updmaxqlen(Ctlr *ctlr, Qpair *qp, SDreq *r)
{
	int qlen;
	short *qlenp;
	...
	if (r == nil)
		return;
	qlen = (qp->sidx.tl + qp->sqlen - qp->sidx.hd) % qp->sqlen;
	qlenp = &ctlr->maxqlen[r->write];
	if (qlen > *qlenp)
		*qlenp = qlen;
}
 ...
 */
static void
sendcmd(Ctlr *ctlr, Qpair *qp, Cmd *qtl, Transfer *xfr, SDreq *r)
{
	int qid;
	...
	ctlr->inflight++;
	iosttck = sys->ticks;
	*doorbellsqtl(ctlr, qid) = qp->sidx.tl;		/* start i/o */
	coherence();
...
/* not quite right for admin commands, but harmless */
static int
lbaoverlapsioq(Ctlr *ctlr, Qpair *qp, uvlong lba, uint len)
{
	uint idx, sqlen;
	uvlong slba;
	Cmd *cmd;
	...
	sqlen = qp->sqlen;
	for (idx = qp->sidx.hd; idx != qp->sidx.tl; idx = (idx+1) % sqlen) {
		cmd = &qp->q[idx];
		slba = cmd->slba;
		if (lba < slba + cmd->length + 1 && slba < lba + len) {
			ctlr->overlaps++;
			return 1;	/* new request overlaps queued one */
		}
	}
	return 0;
}
...
/*
 * call with ctlr->issuelock held.  it will be released and reacquired.
 * ensure sub q (qp) space, drain if new request's lba overlaps the sub q's lbas
 * to avoid possible read-before-write, and return with ctlr->issuelock held.
 */
static void
qdrainilock(Ctlr *ctlr, Qpair *qp, uvlong lba, uint len)
{
	int trips;

	for (trips = 15000; trips > 0; trips--) {
		if (!QFULL(qp) && !lbaoverlapsioq(ctlr, qp, lba, len))
			return;

		/* very uncommon case */
		iunlock(&ctlr->issuelock); /* allow interrupts on this cpu */

		/*
		 * we hope that the sub q is draining, but make sure we don't
		 * get stuck here if it's disabled during shutdown, say.
		 */
		qlock(ctlr);
		ctlr->drainsleeps++;
		tsleep(ctlr, qnotfull, qp, 1);
		qunlock(ctlr);

		ilock(&ctlr->issuelock);
	}
	if (trips <= 0)
		panic("nvme: qdrainilock stuck");
...
static void
incomplete(Ctlr *ctlr, Transfer *xfr, Cmd *qtl)
{
	/*
	 * i/o didn't complete after several seconds,
	 * so what state is the controller in?
	 */
	isfatal(ctlr, "nvmeissue");
	panic("sd%C0: nvmeissue: cmd id %d op %#x didn't complete in %d s.",
		ctlr->sdev->idno, xfr->cmdid, qtl->opcode, Timeout/1000);
}
	...
	uvlong lba, uint len)
{
	...
	/* serialise composition of cmd in place at sub q tail. */
	ilock(&ctlr->issuelock);
	/* try to quickly get to issuing the command. */
	if (!QEMPTY(qp) && (QFULL(qp) || lbaoverlapsioq(ctlr, qp, lba, len)))
		qdrainilock(ctlr, qp, lba, len);	/* rare case */

	/* Reserve a space and update sub. q tail index past it. */
	qtl = &qp->q[qp->sidx.tl];
	qp->sidx.tl = sqidxinc(qp, qp->sidx.tl);
	...
	sendcmd(ctlr, qp, qtl, xfr, r);			/* start cmd */
	iunlock(&ctlr->issuelock);
	...
	poperror();
	if (!xfr->done) {
		/* we see missing intrs with the Samsung 983 DCT. */
		ctlr->timeouts++;
		nvmeinterrupt(nil, ctlr);
		if (!xfr->done)
			incomplete(ctlr, xfr, qtl);
	}
...
nvmertopctl(SDev *sdev, char *p, char *e)
{
	...
	return seprint(p, e, "sd%c nvme regs %#p irq %d: max q lens: rd %d/%d "
		"wr %d/%d; avg µs: rd %lld wr %lld; ops %lld timeouts %ld drainsleeps %lld overlaps %lld\n",
//		" implaus %lld %lld"
		sdev->idno, ctlr->port, ctlr->irq,
		ctlr->maxqlen[Read], ctlr->sqlen,
		ctlr->maxqlen[Write], ctlr->sqlen,
		ctlr->totus[Read]  / nonzerocnt(ctlr, Read),
		ctlr->totus[Write] / nonzerocnt(ctlr, Write),
//		ctlr->implaus[Read], ctlr->implaus[Write],
		ctlr->count[Read] + ctlr->count[Write],
		ctlr->timeouts, ctlr->drainsleeps, ctlr->overlaps);
}
...
nvmreset(Regs *regs)
{
	if (regs->cap & Nssrs) {		/* nvm subsys reset avail.? */
...
nvmeprobe(Pcidev *p)
{
	...
	/* this restriction may no longer be needed. test in a vm? */
	if (logstride != 0)
		panic("nvmeprobe: lg doorbell stride must be 2 (for now), not %d",
			2 + logstride);
	ctlr->stride = 1 << (2 + logstride);	/* 2^(2+logstride) */
	// nvmreset(regs);			/* nvm subsys reset avail.? */
...
zeroqhdtls(Qpair *qp)
{
	qp->cidx.hd = qp->sidx.tl = 0;
	qp->cidx.tl = qp->sidx.hd = 0;	/* paranoia */
	coherence();
}

/*
 * return malloced id page, copy id strings into ctlr.
 * return ctlrid->nns via nnsp.
 */
static char *
getidpage(Ctlr *ctlr, int *nnsp)
{
	Ctlrid *ctlrid;

	ctlrid = mallocalign(BY2PG, ctlr->pgsz, 0, 0);
	if (ctlrid == nil)
		panic("nvmectlrenable: out of memory");
	if (nvmeadmissue(ctlr, Admid, Nsall, ctlrid) != 0)
...
nvmectlrenable(Ctlr* ctlr)
{
	...
	nvmeintron(ctlr->sdev);

	nsid = (Nsid *)getidpage(ctlr, &nns);
	...
	/* find a suitable namespace, copy parameters into ctlr */
	gotns = bestns(ctlr, nns, nsid, &bestlba);	/* fills in nsid page */
	if (gotns == 0)
		panic("nvmectlrenable: no suitable namespace found");

	/* remember these for SDunit */
	ctlr->secsize = 1 << nsid->lbafmt[bestlba].lglbasize;
	ctlr->sectors = nsid->cap;
	ctlr->ns = gotns;
	free(nsid);
	if (Debugns)
...
ckstuck(void)
{
	...
	/* seems to aggravate matters when a sata disk goes offline */
	if (TODO && iosttck && sys->ticks - iosttck > 5*HZ && ++whined < 5)
...
nvmedisable(SDev* sdev)			/* disable interrupts for this sdev */
{
	...
	if (ctlr == nil)
		return 1;
	nvmeissue(ctlr, &ctlr->qpair[Qio], nil, Cmdflush, Nsall, nil, 0, 1);
