...
static	int zig[64] = {
	0, 1, 8, 16, 9, 2, 3, 10,	/* 0-7 */
	17, 24, 32, 25, 18, 11, 4, 5,	/* 8-15 */
			...
			for(block=0; block<H[comp]*V[comp]; block++){
				...
				for(;;){
					...
					if((t&0x0F) == 0){
						...
						z = receive(h, t&0xF);
						if (k < 0 || k >= nelem(zig)) {
							fprint(2, "jpg: baselinescan: "
								"k %d out of range 0..%d\n",
								k, (int)nelem(zig)-1);
							break;
						}
						assert((unsigned)k < nelem(zig));
						zz[zig[k]] = z*qt[k];
						if(k == 63)
							break;
						k++;
...
decode(Header *h, Huffman *t)
{
	...
	if(i >= 17){
		if(badcode == 0)
			fprint(2, "jpg: badly encoded %dx%d JPEG file; "
				"ignoring bad value\n", h->X, h->Y);
		badcode = 1;
		i = 0;
	}
