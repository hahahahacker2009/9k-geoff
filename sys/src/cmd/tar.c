...
#define isextattr(lf)	((lf) == 'x' || (lf) == 'g')	/* pax */
...
typedef union {
	...
	struct {
		...
		char	mtime[12];
		char	chksum[8];
		char	linkflag;	/* a.k.a. "typeflag" for ustar or pax */
...
static long
parsecksum(Hdr *hp, char *name)
{
	return hdrotoull(hp->chksum, hp->chksum + sizeof hp->chksum,
		(uvlong)-1LL, name, "checksum");
}

/* returns a pointer into the buffers at tpblk, or a static Hdr */
static Hdr *
readhdr(int ar)
{
	long hdrcksum;
	...
	hdrcksum = parsecksum(hp, name(hp));
...
/*
 * copy a file from the archive into the filesystem (or just list it).
...
extract(char **argv)
{
	...
	while ((hp = readhdr(ar)) != nil) {
		if (isextattr(hp->linkflag)) {
			skip(ar, hp, "skipping extended attr.s");
			continue;
		}
		longname = name(hp);
