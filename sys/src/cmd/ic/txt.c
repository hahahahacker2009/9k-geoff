...
regalloc(Node *n, Node *tn, Node *o)
{
	...
	case TDOUBLE:
		...
		/* reuse registers from F8 to aid instruction compression */
		for(j=NREG+FREGLOWALLOC; j < NREG+FREGEXT-2; j++) {
			if(reg[j] == 0) {
				i = j;
				goto out;
			}
		}
		for(j=NREG; j < NREG+FREGLOWALLOC; j++)
			if(reg[j] == 0) {
				i = j;
				goto out;
			}
...
gmove(Node *f, Node *t)
{
	...
	case TULONG:
		...
		case TCHAR:
		case TUCHAR:
			if(thechar == 'j' && f->op == OCONST && f->vconst != 0)
				a = AMOVW;
			else
				a = AMOV;
			break;
