/*
 * connection server - translate dial string to connect commands
 *
 * added Mfile arguments to permit net4, to dial only ipv4, for gmail.
 */
#include <u.h>
#include <libc.h>
#include <auth.h>
 ...
 */
Ndbtuple*	iplookup(Network*, char*, char*, int, Mfile *);
char*		iptrans(Ndbtuple*, Network*, char*, char*, int);
Ndbtuple*	telcolookup(Network*, char*, char*, int, Mfile *);
char*		telcotrans(Ndbtuple*, Network*, char*, char*, int);
Ndbtuple*	dnsiplookup(char*, Ndbs*, Mfile *mf);

struct Network
{
	char		*net;
	Ndbtuple	*(*lookup)(Network*, char*, char*, int, Mfile *);
...
main(int argc, char *argv[])
{
	...
	ARGBEGIN{
	case '4':
		ipv6lookups = 0;	/* for gmail.com; see net4! instead */
		break;
	case 'd':
		debug = 1;
		break;
...
lookup(Mfile *mf)
{
	...
	if(strcmp(mf->net, "net") == 0 || strcmp(mf->net, "net4") == 0){
		/*
		 *  go through set of default nets
		 */
		for(np = mf->nextnet; np; np = np->next){
			nt = (*np->lookup)(np, mf->host, mf->serv, 1, mf);
			if(nt == nil)
				continue;
 ...
 */
Ndbtuple*
iplookup(Network *np, char *host, char *serv, int nolookup, Mfile *mf)
{
	char *attr, *dnsname;
	Ndbtuple *t, *nt;
	Ndbs s;
 ...
 */
Ndbtuple*
telcolookup(Network *np, char *host, char *serv, int nolookup, Mfile *)
{
	Ndbtuple *t;
	Ndbs s;
 ...
 */
Ndbtuple*
dnsiplookup(char *host, Ndbs *s, Mfile *mf)
{
	char buf[Maxreply];
	Ndbtuple *t;
	...
	else if (!ipv6lookups || mf && mf->net && strcmp(mf->net, "net4") == 0)
 ...
 */
static Ndbtuple*
ipresolve(char *attr, char *host, Mfile *mf)
{
	Ndbtuple *t, *nt, **l;

	t = iplookup(&network[Ntcp], host, "*", 0, mf);
	for(l = &t; *l != nil; ){
		nt = *l;
		if(strcmp(nt->attr, "ip") != 0){
			*l = nt->entry;
