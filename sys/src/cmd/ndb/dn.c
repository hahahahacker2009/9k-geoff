 ...
 */
enum {
	/* don't make these too large, to avoid ulong overflow */
	Deftarget	= 1<<26,	/* effectively disable aging */
	Minage		= 1<<26,
	Defagefreq	= 1<<26,	/* age names this often (seconds) */
...
dnage(DN *dp)
{
	...
	for(rp = dp->rr; rp; rp = next){
		...
		next = rp->next;
		/*
		 * if we expire this RR, what will trigger fetching it again?
		 * Are clients going to be told that there's no such RR in the
		 * mean time?
		 */
...
dnageallnever(void)
{
	...
	dnslog("%ld initial domain names; target is %lld", dnvars.names, target);
	if(dnvars.names >= target)
		dnslog("more initial domain names (%ld) than target (%lld)",
			dnvars.names, target);
}
...
dnageall(int doit)
{
	...
	if(dnvars.names >= target) {
		dnslog("more names (%lud) than target (%llud)", dnvars.names,
			target);
		dnvars.oldest /= 2;
			...
			for(rp = dp->rr; rp; rp = rp->next)
				if(rp->db){
					if(area){
						...
						/*
						 * only if no explicit rr ttl,
						 * use soa's (i.e., use minttl
						 * as default instead).
						 */
						// if(rp->ttl < minttl)
						if(rp->ttl == 0)
...
//	dnslog("rrattach1: %s", new->owner->name);
	if(!new->db) {
		 ...
		 * egregious overuse of ttl (it's absolute time in the
		 * cache [RR->db == 0]).
		 */
