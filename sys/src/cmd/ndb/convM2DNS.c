...
/*
 * gmail.com, for example, sets a very low expire (1800).
 * this shows up on mx rrs, causing us to think that gmail.com has no mxs.
 * force a longer minimum for now.
 */
void
extendexpire(SOA *soa)
{
	if (soa->expire < Day)
		soa->expire = Day;
}
...
retry:
	...
	case Tmx:
		...
		rp->host = dnlookup(NAME(dname), Cin, 1);
		/*
		 * don't do this until we can determine that this was a
		 * UDP reply.  otherwise we could loop.
		 */
		if(0 && rp->host && strchr(rp->host->name, '\n') != nil)
