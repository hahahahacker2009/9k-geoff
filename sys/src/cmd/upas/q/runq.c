...
reportstatus(Dir *dp, Waitmsg *wm, char **av)
{
	...
	if(strstr(msg, "Ignore") != nil){
		...
		syslog(0, runqlog, "no Retry in %s", msg);
...
dofile(Dir *dp)
{
	...
	/* return stuck mail */
	if(!Eflag && NOW() > (ulong)dtime + giveup){
		syslog(0, runqlog, "now %lud > dtime %llud + %lud",
			NOW(), dtime, giveup);
...
returnmail(char **av, char *name, char *msg)
{
	...
	if(fd >= 0){
		...
		fprint(pfd[1], "[error file %s for %s missing: %r]\n",
			file(name, 'E'), name);
	close(pfd[1]);
out:
	wm = wait();
