...
main(int argc, char **argv)
{
	...
	getconf();
	if (isbadguy()) {
//		syslog(0, "smtpd", "dropping connection from %s", nci->rsys);
		exits("banned");
	}
	Binit(&bin, 0, OREAD);
...
addbadip(void)
{
	...
	if (nci == nil || nci->rsys == nil)
		return;
	if (strchr(nci->rsys, '.') == nil)	/* not v4? must be v6 */
		return;
	bads = sysopen("/sys/lib/badips", "cA", 0666);
	if (bads) {
		Bseek(bads, 0, 2);
		Bprint(bads, "%s\n", nci->rsys);
		sysclose(bads);
...
sayhi(void)
{
	...
	if (dp && dp->length > 0) {
		addbadip();
		syslog(0, "smtpd", "Hung up on impatient spammer %s; blocking",
			nci->rsys);
		if(Dflag)
...
Liarliar:
				addbadip();
				syslog(0, "smtpd",
					"Hung up on %s and blocked; claimed to be %s",
					nci->rsys, him);
				if(Dflag)
					sleep(delaysecs()*Second);
				reply("554 5.7.0 Liar!\r\n");
				exits("client lied about its name");
				return;
			}
		}
	}
	...
	if (extended) {
		...
		/* SMTPUTF8 implies ability of our dns to handle Unicode */
		// reply("250-SMTPUTF8\r\n");		/* RFC 6531 */
		if(tlscert != nil)
			reply("250-STARTTLS\r\n");
		if (noauthcmd)
...
/*
 * read (recipient, sender's DNS domain) pairs from /mail/lib/senders.
...
bprintnode(Biobuf *b, Node *p, int *cntp)
{
	...
	*cntp = 0;
	if(p->s){
		if(p->addr && strchr(s_to_c(p->s), '@') == nil){
			...
			*cntp += s_len(p->s) + 1 + strlen(him);
			...
			len = s_len(p->s);
			if(Bnonulwrite(b, s_to_c(p->s), len) < 0)
				return nil;
			*cntp += len;
		}
	}else{
	...
	if(p->white) {
		len = s_len(p->white);
		if(Bnonulwrite(b, s_to_c(p->white), len) < 0)
			return nil;
		*cntp += len;
	}
	return p->end+1;
...
pipemsg(int *byteswritten)
{
	...
	if(status == 0 && Bnonulwrite(fp, cp, nonhdr) < 0){
	...
	while(sawdot == 0){
		...
		nbytes += n;
		if(status == 0 && Bnonulwrite(fp, cp, n) < 0){
...
sendermxcheck(void)
{
	...
	*cp++ = 0;
	...
	if (strstr(w->msg, "rejected:") != nil && strstr(w->msg, "spam") != nil)
		addbadip();
	/*
