...
Biobuf	bin;		/* network connection */
Biobuf	bout;		/* " */
Biobuf	berr;
Biobuf	bfile;		/* standard input */

/* we compare these by address, at least sometimes */
...
void
failsummary(char *addr, String *from, int rcvrs, char **argv, char **errs)
{
	int i, nerrs;
	...
	for(i = 0; i < rcvrs; i++)
		if(errs[i]){
			++nerrs;
			syslog(0, "smtp.fail",
				"delivery to %s at %s from %s failed: %s",
				argv[i], addr, s_to_c(from), errs[i]);
			Bprint(&berr, "  mail to %s from %s failed: %s",
				argv[i], s_to_c(from), errs[i]);
		}
	if (nerrs == 0) {
		syslog(0, "smtp.fail",
			"delivery to %s at %s from %s failed with no error",
			argv[i], addr, s_to_c(from));
		Bprint(&berr, "  mail to %s from %s failed with no error",
			argv[i], s_to_c(from));
	}
}

/*
 * connect to host addr and send message to receivers in argv.
 * assume addr is an mx for the destination, or the destination iff no mxs.
 * ds is provided only for netdir.
 */
char *
smtpsend(char *addr, DS *ds, String *from, int argc, char **argv)
{
	int ok, nretry, nperm, rcvrs;
	char *rv, *trv;
	char **errs;

	/*
	 * if no such dns name or no dns a nor aaaa recs,
	 * this host is hopeless, so give perm error.
	 */
	/* connect sets bin & bout to network conn. to an mx if successful. */
	if((rv = connect(addr, ds)) != 0)
		return rv;
	...
	/* if all statuses match, return that. */
	if (ok == rcvrs)
		rv = nil;
	else if (nperm == rcvrs)
		rv = Giveup;
	else if (nretry > 0)		/* if any retries, return Retry. */
		rv = Retry;
	else if (ok > 0)
		rv = nil;		/* send even if perm. failures */
	else
		rv = Giveup;		/* can't happen? */
	...
	if(rcvrs > ok) {
		/*
		 *  some but not all rcvrs failed
		 */
		failsummary(addr, from, rcvrs, argv, errs);
		rv = Giveup;
	}
	Bterm(&bin);
	Bterm(&bout);
...
/* sweep mx statuses: retry unless all are Giveup. */
static char *
summmxsts(Mx *mx, int nmx)
{
	Mx *mxp;
...
/*
 * get a list of mx entries for ddomain in mx[] and nmx.
 * something is odd with our dns, so ask it a few times if no mx returned
 * (e.g., for gmail.com).
 */
static int
polldnsformxs(DS *dsp)
{
	int tries;
	long oalarm;

	tries = 0;
	while (++tries < 7 && mxlookup(dsp, ddomain) <= 0) {
		if (nmx < 0)
			syslog(0, "smtp", "no dns for %s", ddomain);
		oalarm = alarm(0);	/* don't time out while asleep */
		sleep(30*1000);
		alarm(oalarm);
	}
	return tries;
...
/* call mxs until success or none left */
static char *
callmxs(DS *dsp, String *from, int argc, char **argv)
{
	char *status;
	Mx *mxp;
	...
	for (mxp = mx; mxp < &mx[nmx]; mxp++) {
		...
		setmxsts(mxp, status);
		if(status != nil)
			continue;	/* already failed on this mx */

		/* ds is for netdir */
		status = smtpsend(mxp->host, dsp, from, argc, argv);
		if (status == nil)
			return nil;		/* success */
		setmxsts(mxp, status);
	}
...
char *
tryallmxs(char *addr, String *from, int argc, char **argv)
{
	int tries;
	ulong start;
	char *dsaddr, *rv;
	...
	dsaddr = netmkaddr(addr, 0, "smtp");
	memset(&ds, 0, sizeof ds);
	dial_string_parse(dsaddr, &ds);

	werrstr("");
	start = time(nil);
	tries = polldnsformxs(&ds);
	if(nmx < 0)		/* dns isn't working? don't just dial dest */
		return Retry;

	if (nmx == 0) {			/* call dest directly */
		if(debug)
			Bprint(&berr, "mxlookup returns none; trying %s\n",
				dsaddr);
		syslog(0, "smtp", "no mx for %s; dialing it", ddomain);
		rv = smtpsend(dsaddr, &ds, from, argc, argv);
	} else {			/* call mxs in preference order */
		if (tries > 1)
			syslog(0, "smtp", "got mxs for %s after %d tries (%lud secs)",
				ddomain, tries, time(0) - start);
		if(nmx > 1)
			qsort(mx, nmx, sizeof(Mx), mxcompar);
		if (callmxs(&ds, from, argc, argv) == nil)
			return nil;		/* success */

		/* failed. sweep mx statuses: retry unless all are Giveup. */
		rv = summmxsts(mx, nmx);
	}

	/* fall back to our mail gateway if nothing else works */
	rerrstr(err, sizeof(err));
	if(rv && gdomain && strstr(err, "can't translate") != 0)
		rv = smtpsend(netmkaddr(gdomain, 0, "smtp"),
			&ds, from, argc, argv);
	return rv;
}
...
main(int argc, char **argv)
{
	...
	case 'o':
		...
		break;
	case 'p':
		alarmscale = 30*Second;		/* was 10* then 20* */
		ping = 1;
		break;
	case 's':
		trysecure = 1;
	...
	if(domain == nil)
		Bprint(&berr, "%s: nil domainname_read()\n", argv0);
	if(host == 0)
		host = sysname_read();
	...
	rv = tryallmxs(addr, from, argc, argv);
	exits(rv);
}

static char *
strdef(char **strp, char *defval)
{
	if (*strp == nil || **strp == '\0')
		*strp = defval;
	return *strp;
 ...
 *  sets bin & bout, copes with error statuses
 */
static char *
connect(char* addr, DS *dsdir)
{
	char *net, *netdir;
	char buf[Errlen];
	int fd;
	...
	/*
	 * force ipv4 for a google mx to avoid excessively strict checking
	 * when using ipv6 smtp.
	 */
	strdef(&ds.netdir, dsdir->netdir); /* mx addr will lack netdir & proto */
	if (ds.host && cistrstr(ds.host, "google.com") != nil)
		if (ds.proto == nil || strcmp(ds.proto, "net") == 0 ||
		    strcmp(ds.proto, "tcp") == 0) {
			ds.proto = "net4";		/* known to ndb/cs */
			// syslog(0, "smtp", "forcing ipv4 for google mx %s",
			//	ds.host);
		}

	strdef(&ds.proto, "net");
	strdef(&ds.service, "smtp");
	if (ds.netdir == nil || ds.netdir[0] == '\0')
		netdir = strdup("");
	else
		netdir = smprint("%s/", ds.netdir);
	net = smprint("%s%s!%s!%s", netdir, ds.proto, ds.host, ds.service);
	free(netdir);
	...
	if(fd < 0){
		...
		Bprint(&berr, "smtp: %s (%s): %r\n", buf, net);
		syslog(0, "smtp.fail", "%s (%s): %r", buf, net);
		free(net);
		/*
		 * if no such dns name or no dns a nor aaaa recs,
		 * this host is hopeless, so give perm error.
		 */
		/* fd == -2 from csdial in dial: no ips in dns */
		if(fd == -2 || strstr(buf, "illegal")
		|| strstr(buf, "unknown")
...
chkgreeting(char *me)
{
	...
	case 2:
		 ...
		 */
		line = s_to_c(reply);
		if (strlen(line) >= Len220 &&
		    strncmp(line+Len220, me, strlen(me)) == 0) {
			if (debug)
				Bprint(&berr, "we dialed ourselves for HELO %s; "
					"sender claims to be us\n", me);
			syslog(0, "smtp.fail", "we dialed ourselves for HELO "
				"%s; sender claims to be us", me);
			return Giveup;
		}
		break;
	case 5:
 ...
 */
char *
data(String *from, Biobuf *b, char *mxaddr)
{
	char *buf, *cp;
	...
	if(!filter){
		...
		syslog(0, "smtp", "%s sent %d bytes to %s via mx %s",
			s_to_c(from), nbytes, s_to_c(toline), mxaddr); /**/
	}
	return 0;
}
...
dBprint(char *fmt, ...)
{
	...
	va_end(arg);
	if(debug){
		Bnonulwrite(&berr, buf, (long)(out-buf));
		Bflush(&berr);
	}
	n = Bnonulwrite(&bout, buf, (long)(out-buf));
	Bflush(&bout);
	return n;
}
