...
Filemagic long0tab[] = {
	...
	0407,		0xffff,	"pdp-11 executable\n",	OCTET,
	0410,		0xffff,	"pdp-11 pure text executable\n", OCTET,
	0411,		0xffff,	"pdp-11 split i/d executable\n", OCTET,
	0413,		0xffff,	"vax executable\n",	OCTET,
...
iself(void)
{
	...
	if (memcmp(buf, "\x7fELF", 4) == 0){
		if (!mime){
			...
			if (p == nil) {
				snprint(cpubuf, sizeof cpubuf, "cpu-%d", n);
				p = cpubuf;
			}
			...
			if(n>0 && n < nelem(type))
				t = type[n];
			if (t == nil) {
				snprint(typebuf, sizeof typebuf, "type-%d", n);
				t = typebuf;
			}
			print("%s ELF%s %s\n", p, (buf[4] == 2? "64": "32"), t);
		}
