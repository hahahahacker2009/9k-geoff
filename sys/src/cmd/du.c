...
du(char *name, Dir *dir)
{
	...
	seen(dir);			/* register subtree root */
	fd = open(name, OREAD);
	if(fd < 0)
		return warn(name);
	nk = 0;
...
seen(Dir *dir)
{
	...
	dp = c->cache;
// fprint(2, "seen: %s qid path %#llux type %#x dev %#ux\n", dir->name, dir->qid.path, dir->type, dir->dev);		// TODO
	for(i=0; i<c->n; i++, dp++) {
// fprint(2, "check: %s qid path %#llux type %#x dev %#ux\n", dir->name, dp->qid.path, dp->type, dp->dev);		// TODO
	...
	c->cache[c->n++] = *dir;
// fprint(2, "new: %s qid path %#llux type %#x dev %#ux\n", dir->name, dir->qid.path, dir->type, dir->dev);		// TODO
	return 0;
}
