...
enum
{
	...
	LOGNPTR = LOGBLKSZ-LOGPTR, /* used to assume sizeof(void*)==4, lg==2 */
	NPTR = 1<<LOGNPTR,
};
...
addpart(char *name, vlong start, vlong end)
{
	...
	if(start < 0){
		werrstr("bad partition boundaries: start %lld < 0", start);
		return -1;
	}
	if(start > end){
		werrstr("bad partition boundaries: start %lld > end %lld",
			start, end);
		return -1;
	}
	if(end > nsect){
		nsect = end; /* extend virtual disk; image could be truncated */
		tab[0].length = nsect;
	}
...
main(int argc, char **argv)
{
	...
	if(NPTR != BLKSZ/sizeof(void*))
		sysfatal("unexpected pointer size %d, not %d",
			(int)sizeof(void *), NPTR);
	if(file){
		Dir *dir;
		Disk *disk;
		...
		disk = opendisk(file, rdonly, 1);
		if (disk) {
			sectsize = disk->secsize;
			// nsect = disk->secs;
			s = disk->s;
			h = disk->h;
			c = disk->c;
