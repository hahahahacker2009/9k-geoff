...
getserv(char *proto, char *dir, Service *s)
{
	...
	readstr(dir, "remote", s->remote, sizeof(s->remote));
	if(p = utfrune(s->remote, L'\n'))
		*p = '\0';

	n = readstr(dir, "local", addr, sizeof(addr));
	if(n <= 0)
		return 0;
	if(p = utfrune(addr, L'\n'))
		*p = '\0';
...
/*
 *  read a string from a device dir/info into s of len bytes.
 *  len includes a terminating NUL.
 */
int
