 ...
 * must be able to hold a pointer.  runes are bigger than that now, but
 * the new ones are mostly emojis and hieroglyphics.
 */
enum
{
	Italic	=	16,
...
//	Bprint(&bout, "<style>body { font-size: 14pt; }</style>\n");
	Bprint(&bout, "</head>\n");
...
trailer(void)
{
	...
	Bprint(&bout, "Copyright</A> &#169; %d Plan 9 Foundation</font>\n",
		t->year+1900);
#endif
