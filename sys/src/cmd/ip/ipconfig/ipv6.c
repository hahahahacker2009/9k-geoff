...
issueadd6(Conf *cf)
{
	...
	/* echo add :: /0 v6gaddr >/net/iproute */
	if(beprimary==1 && validip(cf->v6gaddr))
		adddefroute(cf->mpoint, cf->v6gaddr);
}
...
//	issueadd6(&conf);		/* for conf.v6gaddr? */
	...
	while (pktlen - m > 0) {
		...
		case V6nd_pfxinfo:
			...
			if (first) {
				first = 0;
				ralog("initial RA from gw %I on %s; pfx %I/%d",
					ra->src, conf.dev, conf.v6pref,
					conf.prefixlen);
				if (access("/env/v6prefix", AEXIST) < 0) {
					pfx = smprint("%I", conf.v6pref);
					putenv("v6prefix", pfx);
					free(pfx);
				}
			}
