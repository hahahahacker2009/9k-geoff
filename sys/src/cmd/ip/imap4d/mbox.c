 ...
 *
 * the .imp files are locked by /mail/mbox/username/L.mbox
 * (used to claim /mail/fs/usename/L.mbox, but that makes no sense).
...
closeImp(Box *box, MbLock *ml)
{
	...
	for(m = box->msgs; m != nil; m = m->next){
		if(m->expunged)
			continue;
		/* if we garbled the digest, don't write junk into .imp file */
		if (!ishexstr(m->info[IDigest]))
			continue;
		wrImpFlags(buf, m->flags, strcmp(box->fs, "imap") == 0);
		Bprint(&b, "%.*s %.*lud %s\n", NDigest, m->info[IDigest],
			NUid, m->uid, buf);
	}
	Bterm(&b);
...
parseImp(Biobuf *b, Box *box)
{
	...
	while(m != nil){
		...
		/*
		 * it would be better to use gettokens here.
		 * returning 0 on a mal-formed line seems to cause
		 * clients to refresh endlessly.
		 */
