...
enum {
	Sharefs = 0,		/* see comment in setupuser */
};
...
setupuser(AuthInfo *ai)
{
	...
	/*
	 * we have reverted this change because upas/fs seems to
	 * get confused by multiple concurrent sessions (mounts).
	 * it writes to the wrong .imp files and seems to be unsure
	 * which mailbox it's supposed to be using.
	 */
    if (Sharefs) {
	/*
	...
	case -1:
		...
		break;
	case 0:
		execl("/bin/upas/fs", "upas/fs", (Sharefs? "-nps": "-np"), nil);
_exits("rob1");
		_exits(0);
		break;
	default:
