...
static int
isv4name(char *name, char *ip)
{
	int r = 1;
	char *root, *ip4, *ip6, *pr;
	DS ds;
	...
	free(target);
	strcpy(ip, ds.rem);		/* in case it's a literal */
	...
	strcpy(ip, "");
	ip4 = csgetvalue(root, "sys", ds.rem, "ip", nil);
	if (ip4 == nil)
		ip4 = csgetvalue(root, "dom", ds.rem, "ip", nil);
	ip6 = csgetvalue(root, "sys", ds.rem, "ipv6", nil);
	if (ip6 == nil)
		ip6 = csgetvalue(root, "dom", ds.rem, "ipv6", nil);

	if (ip4 && isv6lit(ip4)) {
		strcpy(ip, ip4);
		r = 0;
	} else if ((ip4 == nil || proto == &v6pr) && ip6 && isv6lit(ip6)) {
		strcpy(ip, ip6);
		r = 0;
	} else if (ip4 && isdottedquad(ip4)) {
		strcpy(ip, ip4);
		r = 1;
	}
	free(ip4);
...
main(int argc, char **argv)
{
	...
	if (isv4name(argv[0], ip))
		proto = &v4pr;
	else
		proto = &v6pr;
	ds = netmkaddr(ip, proto->net, "1");
	cfd = -1;
	fd = dial(ds, 0, conndir, &cfd);
	if(fd < 0){
		fprint(2, "%s: couldn't dial %s (%s) with %s: %r\n",
			argv0, argv[0], ip, ds);
		exits("dialing");
	}

	nci = getnetconninfo(conndir, fd);
	if (!quiet)
		print("sending %d %d-byte messages %d ms apart to %s (%s)\n",
			nmsg, msglen, interval, ds, nci->rsys);
	if (cfd >= 0)
