...
#ifndef oldway
					/*
					 * TODO: if reading Qwsysdir & wname is
					 * "wsys", return root's Qwsys.
					 * avoids silly recursion in du.
					 */
					if((FILE(qid) == Qwsysdir ||
					    FILE(qid) == Qwsys ||
					    FILE(qid) == Qdir) &&
					    strcmp(d->name, "wsys") == 0) {
						path = Qwsys;
						id = 0;		/* for Accept */
					} else
#endif
...
#ifndef oldway
	/*
	 * TODO: if path is Qwsys & name is "wsys", return root's Qwsys.
	 * avoids silly recursion in du.
	 */
	if ( /* (dir->qid & 0xff) == Qwsys && */
	    strcmp(dir->name, "wsys") == 0) {
		d.qid.path = Qwsys;
	} else
#endif
