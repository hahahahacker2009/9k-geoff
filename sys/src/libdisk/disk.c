...
//	if(disk->c != 0 || disk->h != 0 || disk->s != 0)
	/* this seems insane, but may be needed for backward compatibility */
	if(disk->c == 0 || disk->h == 0 || disk->s == 0)
		return -1;
	 ...
	 */
	prefix = disk->prefix? disk->prefix: "";
	rawname = malloc(strlen(prefix) + 5);	/* prefix + "data" + nul */
	if(rawname == nil)
		return -1;

	strcpy(rawname, prefix);
	strcat(rawname, "data");
