...
static DigestState*
sha2_128(uchar *p, ulong len, uchar *digest, SHA2_512state *st, int dlen)
{
	int i;
	u64int x[16];
	uchar buf[256];
	...
	s = st;
	/* read digest, work on a copy */
	if(p == nil && st != nil && !st->malloced) {
		len = 0;
		memmove(rs.bstate, st->bstate, sizeof(st->bstate));
		memmove(rs.buf, st->buf, sizeof(st->buf));
		rs.len = st->len;
		rs.blen = st->blen;
		rs.malloced = 0;
		s = &rs;
