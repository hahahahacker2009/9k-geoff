...
/*
 * using fprint to write control messages here and in tlsClient2 is risky:
 * control message boundaries are significant, and the internal vfprint buffer
 * may not be large enough to hold the entire control message.
 */
	...
	int (*trace)(char*fmt, ...), PEMChain *chp)
{
	...
	ctlmsg = smprint("secret %s %s 0 %s", c->digest, c->enc, secrets);
	rv = (ctlmsg == nil? -1: write(c->ctl, ctlmsg, strlen(ctlmsg)));
	free(ctlmsg);
...
tlsClient2(int ctl, int hand, uchar *csid, int ncsid, int (*trace)(char*fmt, ...))
{
	...
	ctlmsg = smprint("secret %s %s 1 %s", c->digest, c->enc, secrets);
	rv = (ctlmsg == nil? -1: write(c->ctl, ctlmsg, strlen(ctlmsg)));
	free(ctlmsg);
