...
eallocimage(Display *d, Rectangle r, int ldepth, int repl, int start)
{
	...
	if(i == nil) {
		fprint(2, "allocimage(%R, %d depth) fails\n", r, ldepth);
		exits("allocimage");
	}
	return i;
}

#define CHAN RGB24		/* was screen->ldepth */
#define DRAWCOL display->white	/* was display->ones */
...
main(void){
	...
	t = eallocimage(display, Rect(0,0,NPJW,NPJW), CHAN, 0, 0);
	loadimage(t, Rect(0,0,NPJW,NPJW), pjwbits, sizeof(pjwbits));
	pjw = eallocimage(display, Rect(0,0,NPJW,NPJW), CHAN, 0, 0);
	draw(pjw, pjw->r, t, nil, ZP);
	loadimage(t, Rect(0,0,NPJW,NPJW), pjwmaskbits, sizeof(pjwmaskbits));
	pjwmask = eallocimage(display, Rect(0,0,NPJW,NPJW), CHAN, 0, 0);
	draw(pjwmask, pjwmask->r, t, nil, ZP);
	draw(pjw, pjw->r, pjw, pjwmask, ZP);
	myscreen = eallocimage(display, screen->r, CHAN, 0, 0);
	freeimage(t);

	for(i=0; i<256; i++)
		color[i] = eallocimage(display, Rect(0,0,1,1), CHAN, 1, i);
	for(;;){
		if(edot==&dot[NDOT]){
			edot=dot+1;
			...
			draw(myscreen, myscreen->r, DRAWCOL, nil, ZP);
		}
		draw(myscreen, myscreen->r, DRAWCOL, nil, ZP);
		for(d=dot;d!=edot;d++)
			upd(d);
