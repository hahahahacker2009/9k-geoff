...
freeup(Click c)
{
	...
	d1 = c.d + 1;
	if(c.d == Depth -1 || (level.board[d1][c.x][c.y].which == None &&
	    level.board[d1][c.x+1][c.y].which == None &&
	    level.board[d1][c.x][c.y+1].which == None &&
	    level.board[d1][c.x+1][c.y+1].which == None))
		return 1;
	return 0;
}
...
cmatch(Click c, int dtop)
{
	...
	cbr = &level.board[c.d][c.x][c.y];
	lc.d = dtop;
	do {
		for(lc.y = 0; lc.y < Ly; lc.y++)
			for(lc.x = 0; lc.x < Lx; lc.x++) {
				lbr = &level.board[lc.d][lc.x][lc.y];
				if(lbr->which == TL && isfree(lc) &&
				    !eqcl(c, lc) && cbr->type == lbr->type)
					return lc;
			}
	} while(--lc.d >= 0);
	return NC;
