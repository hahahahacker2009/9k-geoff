...
/*
 * the 9k kernel uses qmalloc rather than poolalloc, and has no
 * notion of multiple allocation pools.  so far, our 9k systems
 * are 64-bit and 9 systems are 32-bit, so use _BITS64 as a proxy
 * for `avoid pool allocator'.
 */
#ifndef _BITS64
#define POOL_ALLOC
#endif
...
{
	return (ulong*) ((uintptr)byteaddr(i, p) & ~((uintptr)sizeof(ulong)-1));
}
