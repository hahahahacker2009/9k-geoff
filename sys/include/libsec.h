...
enum
{
	...
	AESmaxkey=	32,
	AESmaxrounds=	14,
/* TODO we don't support custom tag sizes */
	AESGCMtagsz=		16,
	AESGCMstdnoncesz=	12,
};
...
void	setupAESGCMstate(AESstate *s, uchar key[], int keybytes);
void setupAESGCMstateWithNonceSz(AESstate *s, uchar key[], int keybytes);
int	aesGCMencrypt(uchar *p, int len, uchar *nonce, int noncelen, uchar *ad, int adlen, AESstate *s);
int	aesGCMdecrypt(uchar *p, int len, uchar *nonce, int noncelen, uchar *ad, int adlen, AESstate *s);
 ...
 * constant time
*/
int	consttime_cmp(uchar, uchar);
int	consttime_bcmp(void*, void*, ulong);

void bxor(uchar *out, uchar*, uchar*, int);
